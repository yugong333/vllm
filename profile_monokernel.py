...
def _make_monokernel_call(bs, n_half, k, num_experts, top_k,
                          x_full, logits_full, w13_fp8, s13, w2_fp8, s2):
    ...
    def _call():
        return op(x, logits, w13_fp8, s13, w2_fp8, s2, scratchpad,
                  top_k=top_k, scoring_func="softmax", renormalize=True)

    return _call


def _make_triton_call(bs, top_k, x_full, logits_full,
                      w13_fp8, s13, w2_fp8, s2):
    ...
    def _call():
        topk_w, topk_ids, _ = fused_topk(
            hidden_states=x, gating_output=logits, topk=top_k,
            renormalize=True,
        )
        return fused_experts_impl(
            hidden_states=x, w1=w13_fp8, w2=w2_fp8,
            topk_weights=topk_w, topk_ids=topk_ids,
            activation="silu", use_fp8_w8a8=True, use_int8_w8a8=False,
            use_int8_w8a16=False, use_int4_w4a16=False,
            per_channel_quant=False, w1_scale=s13, w2_scale=s2,
...
def profile_one_bs(path, x_full, logits_full, w13_fp8, s13, w2_fp8, s2,
                   bs, n_half, k, top_k, num_experts, model_name,
                   warmup, iters, use_graph=False):
...
def main():
    ...
    ap.add_argument("--model", choices=sorted(MODELS.keys()), default="qwen3.5",
    ...
    ap.add_argument("--warmup", type=int, default=3,
                    help="Warmup iterations per BS (not profiled).")
    ...
    print(f"[profile] model={args.model} ({cfg['display_name']}) "
          f"E={E} N_fused={N_FUSED} K={K} top_k={top_k}")
    print(f"[profile] device={torch.cuda.get_device_name(0)} "
    ...
    for bs in batch_sizes:
        for path in paths:
            profile_one_bs(path, x_full, logits_full,
                           w13_fp8, s13, w2_fp8, s2,
                           bs, N_HALF, K, top_k, E, args.model,
                           args.warmup, args.iters, use_graph=args.graph)
