...
def FusedMoE(
    ...
    # -------------------------------------------------------------
    # MoE layer summary log
    # Logs, per FusedMoE layer:
    #   1. Routing weight computation method (sigmoid / softmax / ...)
    #   2. Quantization method + per-projection strategy
    #      (gate/up share w13, down is w2)
    #   3. num_experts / top_k / hidden_size / intermediate_size
    # -------------------------------------------------------------
    def _describe_moe_quant(qm) -> tuple[str, str, str, str, str]:
        """Return (method_name, gate_w, up_w, down_w, act_strategy)."""
        method_name = qm.__class__.__name__

        def _normalize_strategy(s: str) -> str:
            s = s.lower().replace("quantizationstrategy.", "")
            if s == "tensor":
                return "per-tensor"
            if s == "channel":
                return "per-channel"
        ...
        weight_quant = getattr(qm, "weight_quant", None)
        strategy = getattr(weight_quant, "strategy", None)
        if strategy is not None:
            w_strategy = _normalize_strategy(str(strategy))
        elif getattr(qm, "block_quant", False) or getattr(
            qm, "weight_block_size", None
        ):
            block = getattr(qm, "weight_block_size", None)
            w_strategy = f"block-wise{list(block)}" if block else "block-wise"
        elif "Unquantized" in method_name:
            w_strategy = "unquantized"
        else:
            w_strategy = "unknown"

        # -- Activation quantization strategy --------------------------
        # 1) compressed-tensors path: input_quant.strategy + .dynamic
        input_quant = getattr(qm, "input_quant", None)
        if input_quant is not None:
            in_strategy = _normalize_strategy(str(input_quant.strategy))
            dynamic = getattr(input_quant, "dynamic", None)
            # `dynamic` may be True / False / None / a dict — collapse
            # to a simple static|dynamic label.
            is_static = dynamic is False or dynamic is None
            a_strategy = f"{in_strategy}-{'static' if is_static else 'dynamic'}"
        else:
            # 2) vLLM built-in Fp8MoEMethod path
            quant_cfg = getattr(qm, "quant_config", None)
            act_scheme = getattr(quant_cfg, "activation_scheme", None)
            if getattr(qm, "block_quant", False):
                # block-wise fp8 is always dynamic-per-token-1x128 in vLLM
                a_strategy = "per-token-dynamic-1x128"
            elif act_scheme is not None:
                # activation_scheme in {"static", "dynamic"} for Fp8MoEMethod
                a_strategy = f"per-tensor-{act_scheme}"
            elif "Unquantized" in method_name:
                a_strategy = "unquantized"
            else:
                a_strategy = "unknown"

        # gate & up share w13, down is w2; they use the same weight strategy.
        return method_name, w_strategy, w_strategy, w_strategy, a_strategy

    _qm_name, _gate_s, _up_s, _down_s, _act_s = _describe_moe_quant(
        routed_experts.quant_method
    )

    logger.info_once(
        "[FusedMoE %s] routing=%s (scoring_func=%s, use_grouped_topk=%s, "
        "renormalize=%s) | quant_method=%s | weight strategy gate=%s up=%s "
        "down=%s | activation strategy=%s | num_experts(global/local)=%s/%s "
        "top_k=%s hidden_size=%s intermediate_size=%s (per_partition=%s)",
        prefix or "<no-prefix>",
        # 1) routing weight computation
        "sigmoid"
        if scoring_func == "sigmoid"
        else ("softmax" if scoring_func == "softmax" else scoring_func),
        scoring_func,
        use_grouped_topk,
        renormalize,
        # 2) quantization method + strategies
        _qm_name,
        _gate_s,
        _up_s,
        _down_s,
        _act_s,
        # 3) shape/topology info
        global_num_experts,
        routed_experts.local_num_experts,
        top_k,
        hidden_size,
        intermediate_size,
        routed_experts.intermediate_size_per_partition,
    )
