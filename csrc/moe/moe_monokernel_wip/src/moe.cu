...
///////////////////////////////////////////////////////////////////////////////
//
// TMA-accelerated kernel variants
//
// These use TMA for weight tile copies while keeping the existing MMA
// compute path.  Activation copies that use indirect (gathered) addressing
// still use copy128.
//
///////////////////////////////////////////////////////////////////////////////

/**
 * @brief TMA-accelerated BS8 kernel.
 *
 * Same pipeline as moe_kernel_topk_BS8 but weight prefetches use TMA.
 * Phase 1 (activation fetch) still uses copy128 because tokens are
 * gathered by index.
 */
template <typename Dims>
__device__ void moe_kernel_topk_BS8_tma(
    const A_element* __restrict__ activations_in, std::uint32_t batch_size,
    const __nv_bfloat16* __restrict__ router_logits,
    const W_element* __restrict__ expert_weights_up,
    const S_element* __restrict__ expert_scales_up,
    ...
    ScoringFunc scoring_func, bool renormalize,
    MoEGemmSpec<Dims>* __restrict__ spec, MoE_SHM<Dims>* __restrict__ shmem,
    const MoETmaDescriptors& tma_descs) {
  static_assert(Dims::BS <= 8);
  using CoreDims = MoECoreDims<Dims>;

  // Initialize TMA barriers (one thread only)
  if (threadIdx.x == 0) {
    tma_init_barriers<Dims>(shmem);
  }
  __syncthreads();
  ...
  // ── Phase 1: prefetch activations into w[0].orig || routing ─────────────
  // Activations use copy128 (gathered by token index)
  if (is_prefetch_warp<Dims>()) {
    const std::uint32_t pw = get_prefetch_warp<Dims>();
    for (std::uint32_t tok = pw; tok < batch_size;
         tok += CoreDims::PREFETCH_WARP_COUNT)
  ...
  // ── Phase 2: quantize w[0].orig → a.up || TMA prefetch w_up[0] → w[1] ──
  // Use TMA for weight prefetch instead of copy128
  {
    // TMA: one thread issues the copy, barrier tracks completion
    uint64_t* bar = &shmem->tma_bar[0];
    tma_request_up_expert_full<Dims>(tma_descs, shmem->experts[0].id,
                                     shm->w[1].up, bar);
    // Non-TMA threads arrive on the barrier
    if (threadIdx.x != CoreDims::CALC_WARP_COUNT * CoreDims::THREADS_PER_WARP) {
      if (is_prefetch_warp<Dims>()) {
        // Prefetch warps that didn't issue TMA just arrive
      }
    }
  }
  ...
  // Wait for TMA weight prefetch to complete
  tma_barrier_wait(&shmem->tma_bar[0], 0);
  __syncthreads();

  // ── Phase 3: Up-projection — all experts, double-buffered w[].up ────────
  // For TMA, we use the same up-projection function but the weight prefetch
  // inside it still uses copy128 for now (the inner loop prefetches are
  // interleaved with MMA and harder to convert).
  // TODO: Convert inner-loop weight prefetches to TMA in a follow-up.
  moe_up_projection_BS8_allexperts<Dims>(expert_weights_up, expert_scales_up,
                                         top_k, batch_size, spec, shmem);
...
/**
 * @brief TMA-accelerated BS64 kernel.
 *
 * Same pipeline as moe_kernel_topk_BS64 but weight prefetches use TMA.
 */
template <typename Dims>
__device__ void moe_kernel_topk_BS64_tma(
    const A_element* __restrict__ activations_in, std::uint32_t token_count,
    const __nv_bfloat16* __restrict__ router_logits,
    const W_element* __restrict__ expert_weights_up,
    const S_element* __restrict__ expert_scales_up,
  ...
  // Steps 1-3b are identical to the non-TMA path (routing, sorting,
  // quantization don't involve weight copies)
  if (is_calc_warp<Dims>()) {
    topK_BS64<Dims>(top_k, scoring_func, renormalize, router_logits,
                    token_count, shmem);
  }
...

  {
    ...
    for (uint32_t sp = threadIdx.x; sp < virtual_batch; sp += blockDim.x) {
      uint32_t tok = shmem->path.bs64.token_indexes_topk[sp];
      float rw = shmem->path.bs64.token_weights[sp];
      float as = shmem->act_scale[tok];
      shmem->path.bs64.token_weights[sp] = as;
      shmem->topk_weights_flat[sp] = rw;
    }
  }
  __syncthreads();

  // Step 4: up-projection (uses copy128 for activation tiles which are
  // gathered, and copy128 for weight tiles in the inner loop for now)
  // TODO: Convert inner-loop weight prefetches to TMA
  moe_up_projection_topk<Dims>(expert_weights_up, expert_scales_up, spec,
                               shmem);
  cooperative_groups::this_grid().sync();
...
/**
 * @brief TMA-accelerated top-K MoE kernel entry point.
 *
 * Dispatches to TMA variants of BS8/BS64 paths.
 */
template <typename Dims>
__global__ void moe_kernel_topk_tma(
    const A_element* __restrict__ activations_in, std::uint32_t token_count,
    const __nv_bfloat16* __restrict__ router_logits,
    const W_element* __restrict__ expert_weights_up,
    const S_element* __restrict__ expert_scales_up,
  ...
  if constexpr (Dims::BS <= 8) {
    moe_kernel_topk_BS8_tma<Dims>(activations_in, token_count, router_logits,
                                  expert_weights_up, expert_scales_up,
                                  expert_weights_down, expert_scales_down,
                                  activations_out, top_k, scoring_func,
                                  renormalize, spec, shmem, tma_descs);
  } else {
    moe_kernel_topk_BS64_tma<Dims>(activations_in, token_count, router_logits,
                                   expert_weights_up, expert_scales_up,
                                   expert_weights_down, expert_scales_down,
                                   activations_out, top_k, scoring_func,
                                   renormalize, spec, shmem, tma_descs);
