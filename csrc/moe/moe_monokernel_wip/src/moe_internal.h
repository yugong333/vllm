...
struct MoE_SHM {
  ...
  // ── TMA barriers (used when USE_TMA is defined) ──────────────────────────
  // mbarrier objects for TMA async bulk copies.  Double-buffered: phase
  // alternates 0/1 between consecutive uses.  Each barrier tracks the
  // completion of one tile's worth of TMA transfers.
  //
  // Layout: tma_bar[0..1] for weight tiles (double-buffered)
  //         tma_bar[2..3] for activation/temp tiles (double-buffered)
  //
  // When USE_TMA is not defined these are unused but still present to keep
  // the struct layout stable (they cost only 32 bytes).
  static constexpr uint32_t TMA_BAR_COUNT = 4;
  alignas(8) uint64_t tma_bar[TMA_BAR_COUNT];
