...
/**
 * @brief TMA descriptor bundle passed to the kernel.
 *
 * Contains CUtensorMap descriptors for the weight and activation tensors
 * that TMA can copy as rectangular tiles.  Created on the host via
 * tma_create_tensor_map_2d() and passed as a kernel argument.
 *
 * When USE_TMA is not defined the kernel ignores this struct, but it is
 * always present in the interface so that the wrapper can be compiled
 * unconditionally.
 */
struct MoETmaDescriptors {
  // Up-projection weights: [E * 2*N, K] fp8, tile = [W_UP_TILE/2, K] or
  //                        [W_UP_TILE/2, K/2] depending on path
  CUtensorMap w_up;

  // Down-projection weights: [E * K, N] fp8, tile = [W_DOWN_TILE, N]
  CUtensorMap w_down;

  // Down-projection scales: [E * K] fp32, tile = [W_DOWN_TILE]
  // (1D tensor map)
  CUtensorMap s_down;

  // Quantized activations: [BS, K] fp8, tile = [A_TILE, K/2]
  CUtensorMap a_up;

  // Temp fp32 buffer: [TEMP_ROWS, N] fp32, tile = [T_TILE, N]
  CUtensorMap t_down;

  // BS8 original bf16 activations: [BS, K] bf16, tile = [T_TILE, K]
  CUtensorMap a_orig;

  // BS8 temp bf16 intermediates: [TEMP_ROWS, N] bf16, tile = [T_TILE, N]
  CUtensorMap t_bf16;
};
...
/**
 * @brief TMA-accelerated variant of moe_kernel_topk.
 *
 * Identical to moe_kernel_topk but uses TMA (Tensor Memory Accelerator)
 * for global→shared memory copies of weight tiles.  Requires sm_90a.
 *
 * The @p tma_descs parameter contains pre-built CUtensorMap descriptors
 * created on the host.  The kernel uses these to issue hardware-accelerated
 * bulk copies instead of per-thread copy128() loops.
 *
 * All other parameters are identical to moe_kernel_topk.
 */
template <typename Dims>
__global__ extern void moe_kernel_topk_tma(
    const A_element* __restrict__ activations_in, std::uint32_t token_count,
    const __nv_bfloat16* __restrict__ router_logits,
    const W_element* __restrict expert_weights_up,
    const S_element* __restrict expert_scales_up,
