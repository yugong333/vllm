...
#pragma once
  ...
  //
  // Epilogue-isolation flag (down-projection):
  //
  //   MONO_PROFILE_SKIP_DOWN_EPILOGUE  : elides ONLY the down-proj
  //     per-expert epilogue — the `final_d → down_out` SHM writeback,
  //     the `rank_for_tok` compute, the deferred `out_accum += down_out`
  //     accumulate (both the in-K-loop and the final-expert variants),
  //     and the GM `atomicAdd` of `out_accum → down_partial_out`.  The
  //     K-loop itself (weight + activation TMA, WGMMA, scale-apply) is
  //     KEPT, and a `volatile` sink consumes `final_d` so nvcc does not
  //     dead-code the WGMMA chain.  Use it to measure the down phase's
  //     pure streaming compute + memory throughput without the
  //     per-expert boundary gaps (writeback / accumulate / atomicAdd).
  //     Output is INCORRECT — timing/NCU comparison only.  Mirrors
  //     MONO_PROFILE_SKIP_UP_EPILOGUE on the up-projection side.
  //
...
// ── Down-proj weight/activation TMA pipeline depth opt-in detection ─────
// `Dims::KernelConfig::DOWN_PIPE_DEPTH` is optional; default to 2 (the
// classic double-buffer used by every existing Dims tag, including
// BS8 — keeps its SHM footprint and SASS byte-identical, Req 13.3).
// Set to 4 in a Dims variant (e.g. the BS16 tag) to widen the
// down-projection's weight + activation SHM buffers to a 4-deep
// rolling TMA pipeline.  When paired with `K_STEP_DOWN = 128` the four
// slots each hold one 128-K × DOWN_COL_TILE fp8 weight tile, so the
// total weight-buffer SHM is unchanged (4 × 32 KB == 2 × 64 KB) but
// the launcher can keep up to 4 tiles in flight and roll continuously
// across expert boundaries, raising sustained memory-level parallelism
// for the memory-bound down phase.
//
// Must be a power of two so `s & (DOWN_PIPE_DEPTH - 1)` selects the
// ring slot.  Only 2 and 4 are exercised; the down-proj K-loop has a
// dedicated 4-deep launcher gated on `DOWN_PIPE_DEPTH == 4` and falls
// back to the 2-deep double buffer otherwise.
template <typename Dims>
...
struct MoEGemmSpec {
  ...
  struct {
    ...
    // ── Down-projection epilogue breakdown ────────────────────────────
    //
    // Splits the down-proj per-expert "boundary" work (the part that
    // MONO_PROFILE_SKIP_DOWN_EPILOGUE elides, ~10% of kernel time) into
    // its constituent pieces so we can attribute the degradation.
    // Only meaningful when MONO_PROFILE_PHASE_TIMING is defined;
    // otherwise the slots stay 0.  All placed BEFORE t_after_phase5 so
    // the auto-detect's last-field anchor stays on t_after_phase5.
    //
    // Expert-0 epilogue split (recorded on warp 0 lane 0 == thread 0;
    // these run AFTER expert 0's K-loop, the deferred accumulate of
    // expert 0 is NOT here — it runs in expert 1's iter-0 PF body):
    //   t_down_e0_after_writeback : after the final_d → down_out SHM
    //       writeback.  Δ to t_down_after_expert0_kloop = writeback
    //       cost (calc-warp SHM stores).
    //   t_down_e0_after_rank      : after the rank_for_tok compute
    //       (8 threads, 376 idle).  Δ from after_writeback = rank
    //       cost.  Δ from after_rank to t_down_after_expert0_accum =
    //       the trailing __syncthreads cost (waits for the slowest
    //       warp — exposes divergence between the 8-thread rank
    //       compute and the idle warps).
    int64_t t_down_e0_after_writeback;
    int64_t t_down_e0_after_rank;
    // Deferred-accumulate overlap probe (expert e1 = down_group +
    // DOWN_GROUPS, i.e. the SECOND expert this block processes).  The
    // deferred accumulate of the PREVIOUS expert runs on prefetch warps
    // at iter 0 of e1, concurrently with calc warps' iter-0 WGMMA.
    // Comparing the two windows tells us whether the accumulate is
    // truly hidden:
    //   * calc window (warp 0 lane 0):
    //       t_down_e1_iter0_after_wait    : after the iter-0 bar_w /
    //           bar_a mbarrier waits.
    //       t_down_e1_iter0_after_compute : after the iter-0 WGMMA +
    //           scale-apply substep loop.  Δ = calc compute window.
    //   * PF window (warp 8 lane 0 == threadIdx.x == 256):
    //       t_down_e1_pf_before_accum : immediately before the
    //           deferred accumulate loop.
    //       t_down_e1_pf_after_accum  : immediately after.  Δ = the
    //           deferred accumulate cost.
    //   PF Δ ≤ calc Δ → accumulate hidden (calc is the long pole).
    //   PF Δ > calc Δ → calc waits for PF at the iter-0 __syncthreads;
    //                   the "deferred" accumulate is still on the
    //                   critical path.
    ...
    // Final-expert accumulate isolation (recorded on thread 0).  The
    // LAST expert in the group has no next expert to defer to, so its
    // accumulate runs unconditionally after the expert loop.
    //   t_down_before_final_accum : right after the expert loop, before
    //       the final accumulate block.  Δ to t_down_after_all_experts
    //       = final-expert accumulate + its trailing __syncthreads.
    int64_t t_down_before_final_accum;
    // Steady-state (expert e1) epilogue probes — confirms whether the
    // rank_for_tok cost measured on expert 0 (cold SHM) persists once
    // topk_ids_flat / sorted_slot are L1-resident.  rank Δ =
    // t_down_e1_after_rank - t_down_e1_after_writeback.
    int64_t t_down_e1_after_writeback;
    int64_t t_down_e1_after_rank;
...
struct MoECoreDims {
  ...
  // Down-proj weight/activation TMA pipeline depth (ring-buffer slot
  // count).  2 = classic double buffer (default, BS8); 4 = the BS16
  // 4-deep rolling pipeline.  Must be a power of two so the K-loop's
  // `s & (DOWN_PIPE_DEPTH - 1)` ring-slot index folds to a mask.
  static constexpr std::uint32_t DOWN_PIPE_DEPTH = down_pipe_depth<Dims>::value;
  static_assert((DOWN_PIPE_DEPTH & (DOWN_PIPE_DEPTH - 1u)) == 0u,
                "DOWN_PIPE_DEPTH must be a power of two.");
  ...
  // T_TILE is the per-block token-tile width consumed by the FP8
  // routing-quantize / up-projection path.  It is NOT a raw function of
  // Dims::BS for the legacy paths — it is gated so the BS<=8 WGMMA+TMA
  // path keeps the historical value 8 (preserving BS8 SASS bit-identity,
  // Req 13.3) while the BS=16 instantiation widens to 16 so
  // `fp8_act_full[...][T_TILE+1][...]` actually holds all 16 token rows
  // of FP8 (Req 6.2).  The BS=8 expansion still resolves to 8u, so every
  // downstream consumer (FP8_ACT_T_TILE_PADDED = T_TILE + 1 = 9, the
  // up-proj B-descriptor stride, the deferred-epilogue scratch extents)
  // observes the same value as before on the BS8 path.
...
struct MoE_SHM {
  ...
  union U {
    ...
    struct TinyDataWGMMA_TMA {
      ...
      // dependent extents of the new fields to 1 when the Dims tag is
      // not a BS<=16 TMA tag and to their natural value otherwise.
      // The live extent is enabled for any TMA Dims tag with
      // `BS <= 16` (both the BS8 and BS16 WGMMA+TMA tags); a BS64-style
      // tag with `Dims::BS > 16` collapses the extents to 1.  The
      // runtime BS8 dispatch never reads the BS64-instantiated views;
      // the BS64 dispatch never reads `tiny_wgmma_tma` at all (it uses
      // the `path.bs64` arm of the outer union).
      //
      ...
      union {
        ...
        //
        // Sizing (per slot):
        //   w_wgmma[4][W_WGMMA_M_TOTAL][K_STEP_WGMMA=128]
        //     K_STEP_UP=128: W_WGMMA_M_TOTAL=128 → 16 KB per slot
        //                    × 4 slots = 64 KB total
        //     K_STEP_UP=256: W_WGMMA_M_TOTAL=256 → 32 KB per slot
        //                    × 4 slots = 128 KB total (2 substeps ×
        //                    128 M rows × 128 K bytes stacked along M)
        //   w_down_wgmma[2][W_DOWN_WGMMA_M_TOTAL][K_STEP_WGMMA=128]
        //     DOWN_COL_TILE=128, K_STEP_DOWN=128: 16 KB per slot
        //                                          × 2 slots = 32 KB total
        //     DOWN_COL_TILE=256, K_STEP_DOWN=128: 32 KB per slot
        //                                          × 2 slots = 64 KB total
        //     DOWN_COL_TILE=256, K_STEP_DOWN=256: 64 KB per slot
        //                                          × 2 slots = 128 KB total
        //                                          (2 substeps × 256 M rows
        //                                          × 128 K bytes stacked
        //                                          along M)
        //
      ...
      alignas(16) uint64_t bar_a[CoreDims::DOWN_PIPE_DEPTH];  // 16/32 B
      ...
      // Down-proj slot→token inverse map (BS16 only).  Built ONCE in the
      // routing Phase C (where `sorted_slot[pair] = slot` is written) by
      // the companion scatter `slot_to_token[slot] = pair / top_k`, and
      // read read-only by the down-proj accumulate.  It lets the
      // accumulate iterate the expert's routed RANKS directly
      // (`out_accum[slot_to_token[expert_start+rank]][col] +=
      // down_out[col][rank]`) instead of (a) re-deriving each token's
      // intra-expert rank with the per-expert 8-iter `topk_ids_flat`
      // scan, and (b) walking the full BS×col plane and discarding the
      // non-routed tokens.  Because the map is global and immutable for
      // the whole kernel, the deferred accumulate needs NO ping-pong /
      // double-buffer (unlike `rank_for_tok`).
      //
      // Extent collapses to 0 for BS<=8 — that path keeps the original
      // `rank_for_tok` scan, so BS8 SHM size and PTX stay byte-identical
      // (Req 13.3).  Placed at the END of the struct so its
