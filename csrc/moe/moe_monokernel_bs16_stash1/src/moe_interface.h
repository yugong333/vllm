...
// ── BS16 WGMMA kernel — Pair_Layout V2 (gate/up paired in M dim) ───────
// Identical to Dims_BS8_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA except BS and
// M are 16. Reuses the moe_kernel_topk_BS8 template body verbatim
// (Req 1.3) — no separate moe_kernel_topk_BS16 is introduced. Selected
// by host dispatch when 8 < token_count <= 16 (Req 10.2).
struct Dims_BS16_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA {
  static constexpr uint32_t HIDDEN_STATES = 2048;
  static constexpr uint32_t K = 2048;
  static constexpr uint32_t N = 512;
