...
// ── Shared helper functions for the BS8 TMA+WGMMA down-projection ──────────
...
__device__ inline void moe_load_down_wgmma_act_scale_per_expert(
  ...
  static_assert(ScaleTok == CoreDims::T_TILE,
                "down-proj activation scale tile must have T_TILE tokens "
                "(8 at BS<=8, 16 at BS=16 — see CoreDims::T_TILE)");
...
__device__ inline void moe_down_projection_BS8_allexperts_wgmma_tma(
  ...
  // Weight/activation TMA pipeline depth: 2 = classic double buffer
  // (BS8, K_STEP_DOWN=256); 4 = the BS16 rolling 4-deep pipeline
  // (K_STEP_DOWN=128).  The K-loop ring-slot index is `s & (DEPTH-1)`,
  // and a dedicated 4-deep launcher (gated on `DOWN_PIPE_DEPTH == 4`)
  // mirrors the up-projection's 2-iter-lookahead + cross-expert stitch.
  ...
  // Per-K-step activation TMA transfer size.  Each SWZ128 atom covers
  // one 128-K sub-block of `T_TILE` token rows; the atom byte size
  // equals `T_TILE * FP8_ACT_NUM_CHUNKS * FP8_ACT_K_CHUNK` =
  //   * BS<=8 (T_TILE=8):  8 *  8 * 16 = 1024 B
  //   * BS=16 (T_TILE=16): 16 * 8 * 16 = 2048 B
  // The expression below is symbolic so it auto-scales with the
  // BS=16 `T_TILE` widening (task 5.4).  An outer K-step issues
  ...
  // B descriptor strides for the fp8 activation tile under SWIZZLE_128B
  // (token-major Major::K B128 canonical layout).  The TMA hardware
  // applies the 8-row × 128-byte XOR at write time:
  ...
  //   SBO         (stride between consecutive 8-token N-core-matrices)
  //   swizzle = 1  (SWZ128)
  //
  // SBO fix for the BS16 (m64n16k32) hi-quadrant: the WGMMA reads N=16
  // tokens as TWO 8-token N-core-matrices.  The second atom (tokens
  // 8..15 → output N-cols 8..15) is one SWZ128 tile away = 8 tokens ×
  // (FP8_ACT_NUM_CHUNKS*FP8_ACT_K_CHUNK = 128 B/token) = 1024 B.  The
  // legacy 128 B value made the hardware fetch the second N-group
  // starting at token 1 (offset 128), corrupting every rank >= 8 — only
  // observable when an expert has routed_count > 8 (a hot expert), which
  // is why uniform random routing missed it but real skewed routing hits
  // it.  BS8 (m64n8k32) has a single 8-token atom so SBO is unused; the
  // 128 B value is preserved there to keep BS8 SASS byte-identical
  // (Req 13.3).
  ...
  // BS=16 only: companion 4-wide fragments for the m64n16k32 N=[8,16)
  // quadrant (token ranks 8..15).  The m64n16k32 WGMMA produces 8 fp32
  // accumulators per thread; d[0..3] live in `chunk_d_*` / `final_d`
  // and d[4..7] live in the `_hi` companions below.  Declared
  // unconditionally so the source compiles for both Dims tags; on the
  // BS<=8 path every read/write of these arrays is gated under
  // `if constexpr (Dims::BS == 16)`, so nvcc dead-code-eliminates them
  // and BS=8 SASS bit-identity is preserved (Req 13.2).
  ...
  if (is_tma_launcher_thread<Dims>()) {
    ...
    if constexpr (DOWN_PIPE_DEPTH == 4u) {
      // 4-deep pipeline (BS16) uses ring slots 2 and 3 as well.  The
      // up-proj (Phase 3) also touched bar_w[2..3]; the Phase-3→4 grid
      // sync (expert_barrier site #2) guarantees they are idle here, so
      // re-init to arrival_count=1 is safe.
      mbarrier_init(&shm->bar_w[2], 1u);
      mbarrier_init(&shm->bar_w[3], 1u);
      mbarrier_init(&shm->bar_a[2], 1u);
      mbarrier_init(&shm->bar_a[3], 1u);
    }
  ...
  std::uint32_t parity_w[DOWN_PIPE_DEPTH] = {0u};
  std::uint32_t parity_a[DOWN_PIPE_DEPTH] = {0u};
  ...
  #pragma unroll
    ...
    if constexpr (Dims::BS == 16) {
      // BS=16: also reset the second-quadrant accumulator that tracks
      // the m64n16k32 N=[8,16) fragment (token ranks 8..15).
      // Gated to preserve BS=8 SASS bit-identity (Req 13.2).
  #pragma unroll
      for (std::uint32_t h = 0; h < DOWN_COL_HALVES; ++h) {
  #pragma unroll
        for (std::uint32_t r = 0; r < 4u; ++r) {
    ...
    [[maybe_unused]] const bool has_prev_expert = (e != down_group);
    for (std::uint32_t s = 0; s < K_TILES_DOWN; ++s) {
      ...
      if (is_calc) {
        ...
        // Bytes between consecutive `T_TILE` tok × 128 K SWZ128
        // activation atoms (one per K-substep) in `a_down_wgmma[slot]`.
        // T_TILE=8 at BS<=8 → 1024 B; T_TILE=16 at BS=16 → 2048 B.
        // Symbolic so it auto-scales with the T_TILE widening (task 5.4).
        constexpr std::uint32_t B_SUBSTEP_BYTES =
            CoreDims::T_TILE *
            MoE_SHM<Dims>::U::TinyDataWGMMA_TMA::FP8_ACT_NUM_CHUNKS *
    ...
    #pragma unroll
            for (std::uint32_t j = 0; j < 2; ++j) {
              ...
              if constexpr (Dims::BS == 16) {
                // BS=16: single m64n16k32 covers all 16 routed-rank
                // columns (N=[0,16)) per chained issue.  d[0..3] are
    ...
    #pragma unroll
          ...
          if constexpr (Dims::BS == 16) {
            // BS=16: fold the second-quadrant accumulators (token
            // ranks 8..15) into final_d_hi using the corresponding
            // activation scales for those ranks.  Same `ws` as the
            // first quadrant — weight scales are not BS-dependent
            // (Req 8.5).  Gated to preserve BS=8 SASS bit-identity
            // (Req 13.2).
            const std::uint32_t tok_46 = tok_02 + 8u;
            const std::uint32_t tok_57 = tok_13 + 8u;
            const float as_lo_46 = shm->a_down_scale[base_half + 0u][tok_46];
            const float as_hi_46 = shm->a_down_scale[base_half + 1u][tok_46];
            const float as_lo_57 = shm->a_down_scale[base_half + 0u][tok_57];
            const float as_hi_57 = shm->a_down_scale[base_half + 1u][tok_57];
    #pragma unroll
            for (std::uint32_t h = 0; h < DOWN_COL_HALVES; ++h) {
              const float ws = shm->w_down_scale[h][my_wg][ws_col];
      ...
      // deadlock.
      static_assert(DOWN_PIPE_DEPTH != 2u || K_TILES_DOWN % 2u == 0u,
                    "2-deep double buffer requires K_TILES_DOWN to be "
                    ...
                    "offset.");
      static_assert(DOWN_PIPE_DEPTH != 4u ||
                        (K_TILES_DOWN >= 4u && K_TILES_DOWN % 4u == 0u),
                    "4-deep down pipeline requires K_TILES_DOWN to be a "
                    "positive multiple of 4 so the cross-expert stitch at "
                    "s=K_TILES-2 / K_TILES-1 lands the next expert's tiles "
                    "0,1 on ring slots (s+2)&3 == 0,1 — exactly the slots "
                    "the next expert's s=0,1 waits read.  N=512, "
                    "K_STEP_DOWN=128 → K_TILES_DOWN=4.");
      if (is_tma_launcher_thread<Dims>()) {
  #ifndef MONO_PROFILE_SKIP_PREFETCH_DOWN
        if constexpr (DOWN_PIPE_DEPTH == 4u) {
          // ── 4-deep weight + activation lookahead (BS16) ──────────
          // Mirror the up-proj: at iter `s` arm ring slot `(s+2)&3`
          // for the tile its consumer reads 2 iters later, giving the
          // 32 KB weight TMA two compute windows to land.  Three
          // source cases:
          //   (A) intra-expert: s+2 < K_TILES_DOWN → CURRENT expert's
          //       tile (s+2) at k=(s+2)*K_STEP_DOWN.
          //   (B) cross-expert iter-0: s == K_TILES_DOWN-2 → NEXT
          //       expert's tile 0 (k=0) into slot (s+2)&3 == 0.
          //   (C) cross-expert iter-1: s == K_TILES_DOWN-1 → NEXT
          //       expert's tile 1 (k=K_STEP_DOWN) into slot
          //       (s+2)&3 == 1.
          // K_SUBSTEPS_DOWN == 1 (K_STEP_DOWN=128), so each case issues
          // exactly one 32 KB weight TMA + (when routed) one activation
          // atom.  The next expert's priming is therefore already done
          // by the time its loop starts (slots 0,1 from this expert's
          // stitch; slots 2,3 from its own intra arms at s=0,1).
          const std::uint32_t la_slot = (s + 2u) & 3u;
          const bool has_next_s = (s + 2u < K_TILES_DOWN);
          if (has_next_s) {
    ...
    #pragma unroll
            ...
            // (B)/(C) cross-expert stitch into the NEXT expert's tile
            // 0 (s==K_TILES-2 → slot 0) or tile 1 (s==K_TILES-1 →
            // slot 1).  Uses the next expert's id / routed_count /
            // slot_start for the activation operand.
    ...
    #pragma unroll
      ...
      !defined(MONO_PROFILE_SKIP_DOWN_EPILOGUE)
        ...
        //     K-loop).
        if constexpr (Dims::BS == 16) {
          // BS16: per-RANK deferred accumulate via the slot→token
          // inverse map.  Iterate ONLY this (previous) expert's routed
          // ranks — `out_accum[slot_to_token[prev_start+rank]][col] +=
          // down_out[col][rank]` — instead of scanning the full BS×col
          // plane and filtering non-routed tokens.  Most experts route
          // 0..1 tokens, so this collapses the prefetch-warp work from
          // BS*DOWN_COL_TILE (4096) to routed_count*DOWN_COL_TILE, which
          // is the actual down-proj bottleneck (prefetch accumulate >
          // calc WGMMA).  No per-expert rank scan, no 0xFF filter.
          // Still spread across the K_TILES_DOWN iterations so it
          // overlaps the full K-loop compute window.  `down_out` holds
          // the previous expert's results (single-buffered, written at
          // its epilogue); `slot_to_token` is global + immutable so no
          // ping-pong / double-buffer is needed.
          if (has_prev_expert) {
            const std::uint32_t prev_e = e - DOWN_GROUPS;
            const std::uint32_t prev_id = shmem->experts[prev_e].id;
            ...
            const unsigned total_rc = prev_rc * DOWN_COL_TILE;
            const unsigned per_slice =
                (total_rc + K_TILES_DOWN - 1u) / K_TILES_DOWN;
            const unsigned slice_begin = s * per_slice;
            const unsigned slice_end = (slice_begin + per_slice < total_rc)
                                           ? slice_begin + per_slice
                                           : total_rc;
            MONO_PHASE_TIMESTAMP_IF_TID(
                t_down_e1_pf_before_accum,
                e == down_group + DOWN_GROUPS && s == 0u,
                CoreDims::CALC_WARP_COUNT * CoreDims::THREADS_PER_WARP);
            for (unsigned rc = slice_begin + thread_in_pf; rc < slice_end;
                 rc += PF_THREADS) {
            ...
            MONO_PHASE_TIMESTAMP_IF_TID(
                t_down_e1_pf_after_accum,
                e == down_group + DOWN_GROUPS && s == 0u,
                CoreDims::CALC_WARP_COUNT * CoreDims::THREADS_PER_WARP);
          }
        } else {
          // BS8: unchanged single-shot accumulate at iter 0 only
          // (Req 13.3 — BS8 SASS byte-identity preserved).
  ...
  #ifdef MONO_PROFILE_SKIP_DOWN_EPILOGUE
    // Profile-only: keep the WGMMA → scale-apply → final_d dependency
    // chain alive so nvcc doesn't optimize the down-proj K-loop into a
    // no-op when the epilogue (writeback + rank + accumulate + atomicAdd)
    // is elided.  Mirrors the MONO_PROFILE_SKIP_UP_EPILOGUE sink.
    {
      volatile float sink = 0.f;
    #pragma unroll
      for (std::uint32_t h = 0; h < DOWN_COL_HALVES; ++h) {
        sink += final_d[h][0] + final_d[h][1] + final_d[h][2] + final_d[h][3];
        if constexpr (Dims::BS == 16) {
          sink += final_d_hi[h][0] + final_d_hi[h][1] + final_d_hi[h][2] +
                  final_d_hi[h][3];
        }
      }
      (void)sink;
    ...
    #pragma unroll
      for (std::uint32_t h = 0; h < DOWN_COL_HALVES; ++h) {
        ...
        shm->partial_result.down_out[row_base + 8][col_base + 1] =
            final_d[h][3];
        if constexpr (Dims::BS == 16) {
          // BS=16: also write the second-quadrant accumulators
          // (token ranks 8..15) to the +8 / +9 columns of the
          // `down_out[DOWN_COL_TILE][T_TILE]` tile.  At BS=16,
          // `T_TILE = 16` (task 5.4), so `col_base + 8 / +9` is
          // in-bounds (max col = 6 + 9 = 15).  Gated to preserve
          // BS=8 SASS bit-identity (Req 13.2).
    ...
    MONO_PHASE_TIMESTAMP_IF(t_down_e0_after_writeback, e == down_group);
    ...
    // above behind the same trailing __syncthreads().
      ...
      !defined(MONO_PROFILE_SKIP_DOWN_EPILOGUE)
    if constexpr (Dims::BS != 16) {
      // BS<=8: rank_for_tok is computed here in the epilogue and
      // consumed by the (tok,col) accumulate — unchanged, byte-identical
      // (Req 13.3).  BS16 instead uses the routing-built slot→token
      // inverse map in a per-rank accumulate, so this exposed 8-iter
      // scan is dropped entirely.
  ...
  #endif
    MONO_PHASE_TIMESTAMP_IF(t_down_e0_after_rank, e == down_group);
    MONO_PHASE_TIMESTAMP_IF(t_down_e1_after_rank,
                            e == down_group + DOWN_GROUPS);
    __syncthreads();
  ...
  MONO_PHASE_TIMESTAMP(t_down_before_final_accum);
  ...
  if (expert_count > down_group) {
    if constexpr (Dims::BS == 16) {
      // Per-rank final accumulate for the LAST expert this block
      // processed, via the slot→token inverse map (mirrors the deferred
      // accumulate).  All warps participate (the K-loop is done).
      const std::uint32_t last_e =
          down_group +
          ((expert_count - 1u - down_group) / DOWN_GROUPS) * DOWN_GROUPS;
      const std::uint32_t last_id = shmem->experts[last_e].id;
  ...
  #ifndef MONO_PROFILE_SKIP_DOWN_EPILOGUE
