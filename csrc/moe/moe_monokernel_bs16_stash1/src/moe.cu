...
__device__ void moe_kernel_topk_BS8(
  ...
  if (warp_id >= CoreDims::CALC_WARP_COUNT) {
    ...
    // to 2^32 (runaway loop).  Routing is always executed regardless
    // of MONO_PROFILE_SKIP_CALC_{UP,DOWN}; only the per-expert
  ...
  // DOWN_GROUPS=16.  Bounds expressed via `CoreDims` so the code
  ...
  if (down_group_r == 0) {
    ...
    //
    // BS16 FIX: this padding-zero is gated OUT for the BS=16 path.
    // The host allocates `activations_out` with exactly `batch_size`
    // (= num_tokens) rows (see vllm/_custom_ops.py
    // `torch.zeros_like(activations_in)`), NOT `Dims::BS` rows. Writing
    // rows [batch_size, Dims::BS) is therefore an out-of-bounds store
    // past the end of the output allocation. For BS<=8 the runner's
    // CUDA-graph decode buffer is padded to a capture size >= 8, so the
    // stray rows happened to stay in-bounds (and BS8 SASS must remain
    // bit-identical, Req 13). For BS=16 the per-step token count (9..16)
    // can leave the buffer shorter than 16 rows, so the padding write
    // spills into a NEIGHBORING allocation in the shared memory pool —
    // corrupting whatever tensor sits there (observed: a Gated Delta Net
    // index/state tensor, which then dereferences a bad index and raises
    // an illegal address downstream). The consumer only ever reads
    // [0, batch_size), so skipping the padding-zero is functionally
    // correct. `if constexpr (Dims::BS != 16)` is compile-time-true for
    // the BS8 tag → identical codegen (Req 13.2 preserved).
...
__launch_bounds__(Dims::KernelConfig::BLOCK_SIZE, 1) void moe_kernel_topk(
  ...
  // BS16 SHM budget companion (Req 6.1, Req 6.5): the BS16 TMA+WGMMA
  // Dims tag must also fit the 228 KB Hopper opt-in per-block SHM cap.
  // The diagnostic names the tag explicitly so a future overflow points
  // directly at the BS16 layout (Req 6.5).
  static_assert(
      !(use_tma<Dims>::value && Dims::BS == 16) ||
          sizeof(MoE_SHM<Dims>) <= 228 * 1024,
      "Exceeds 228 KB opt-in SHM budget for BS16 TMA+WGMMA "
      "(Dims_BS16_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA). The BS16 path "
      "adds the T_TILE=16 widening across fp8_act_full, a_down_wgmma, "
      "a_down_scale, and the partial_result down_out union dominator, "
      "plus the Dims::BS-driven act_scale / out_accum / topk caches. If "
      "this fires, the BS16 SHM layout has grown past the Hopper opt-in "
      "cap and needs a mitigation (out of scope for this spec).");
  // BS8 sizeof(MoE_SHM) regression anchor (Req 6.3, Req 6.4, Req 13.3):
  // hard-pin the pre-feature BS8 SHM footprint so any BS8 layout drift
  // (which would break SASS bit-identity) fails the build. The literal
  // 175104 is the measured sizeof recorded in baseline/bs8_shm_bytes.txt
  // (task 1.3). Gate on Dims::BS == 8 so this pins only the BS8 tag.
  static_assert(
      !(use_tma<Dims>::value && Dims::BS == 8) ||
          sizeof(MoE_SHM<Dims>) == 175104u,
      "MoE_SHM<Dims_BS8_*> sizeof drifted from the pre-BS16 baseline "
      "(175104 bytes captured in baseline/bs8_shm_bytes.txt); the BS8 "
      "path is no longer SASS-bit-identical (Req 13.3 violation).");

  // ── BS16-vs-BS8 per-tile sizeof scaling anchors (Req 6.2, Req 6.1) ──────
  // These reference the two concrete Dims tags directly (NOT the kernel's
  // `Dims` template parameter) because they compare BS8 against BS16. They
  // are re-checked on every instantiation, which is harmless: the operands
  // are compile-time constants independent of `Dims`.

  // act_scale scales exactly 2x from BS8 to BS16 (common-region member
  // with a Dims::BS extent: [ACT_SCALE_BLOCKS][Dims::BS]). Strict at both
  // the field level and the +512 B MoE_SHM total contribution (Req 6.2).
  static_assert(
      sizeof(moe_monokernel::MoE_SHM<
             moe_monokernel::Dims_BS16_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA>::
                 act_scale) ==
          2 * sizeof(moe_monokernel::MoE_SHM<
                     moe_monokernel::
                         Dims_BS8_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA>::
                         act_scale),
      "act_scale must scale 2x from BS8 to BS16 (Req 6.2).");

  // INFORMATIONAL ONLY: bf16_in_full's literal sizeof doubles (32 KB ->
  // 64 KB), but it is unioned with w_wgmma (128 KB) and w_down_wgmma
  // (128 KB), so the doubling does NOT change sizeof(MoE_SHM<Dims>) — the
  // union max stays at 128 KB (max(64 KB, 128 KB, 128 KB) = 128 KB). This
  // assert documents the field-level 2x scaling, not a MoE_SHM total
  // contribution (Req 6.2).
  static_assert(
      sizeof(moe_monokernel::MoE_SHM<
             moe_monokernel::Dims_BS16_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA>::U::
                 TinyDataWGMMA_TMA::bf16_in_full) ==
          2 * sizeof(moe_monokernel::MoE_SHM<
                     moe_monokernel::
                         Dims_BS8_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA>::U::
                         TinyDataWGMMA_TMA::bf16_in_full),
      "bf16_in_full field sizeof must scale 2x from BS8 to BS16 "
      "(informational; absorbed by the w_wgmma/w_down_wgmma union, "
      "Req 6.2).");

  // fp8_act_full grows by exactly 16384 B from BS8 to BS16. The T_TILE+1
  // row-stride padding makes the literal sizeof ratio ~1.89x (34816 /
  // 18432) rather than 2x, so we pin the absolute delta instead:
  //   BS16: 16 * 8 * 17 * 16 = 34816 B;  BS8: 16 * 8 * 9 * 16 = 18432 B
  //   delta = 16 * 8 * (17-9) * 16 = 16384 B
  // This is the direct (non-union) contribution to sizeof(MoE_SHM) on the
  // BS16 path (Req 6.1, Req 6.2).
  static_assert(
      sizeof(moe_monokernel::MoE_SHM<
             moe_monokernel::Dims_BS16_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA>::U::
                 TinyDataWGMMA_TMA::fp8_act_full) -
              sizeof(moe_monokernel::MoE_SHM<
                     moe_monokernel::
                         Dims_BS8_E256_Qwen3_5_35B_BlockFP8_WGMMA_TMA>::U::
                         TinyDataWGMMA_TMA::fp8_act_full) ==
          16384,
      "fp8_act_full BS16-vs-BS8 sizeof delta must be exactly 16384 B "
      "(Req 6.2).");
  ...
  //
  //   * `grid_counters` — used by grid-wide synchronization sites.
  //   * `expert_counters` — used by site #2 in Phase 2b (task 12.1).
  //     Threaded into `moe_kernel_topk_BS8` so the BS8 Phase-2b
  //     migration can wire it into the Expert_Barrier call without
  //     re-plumbing the call chain.
  //   * `colstripe_counters` — used by site #3 in Phase 2b (task 12.2).
  //     Same plumbing treatment as `expert_counters`.
  ...
  //
  // ELIMINATED. The Phase 5 reduction in moe_kernel_topk_BS8
  // `=`-writes every element of activations_out (assigns reduced sum
  // for tokens [0, batch_size) and explicitly zeros [batch_size,
  // Dims::BS) per block col stripe), so the pre-zero + sync is dead
  // work. See Requirements 3.4, 3.5 and Design Migration Plan Site #1.
