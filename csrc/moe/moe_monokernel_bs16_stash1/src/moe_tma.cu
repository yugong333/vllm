...
CUtensorMap create_activations_tma_desc(const void* activations_ptr,
  ...
  //   boxDim[1]        = batch_size_cap (8 at BS=8, 16 at BS=16)
...
CUtensorMap create_down_activation_tma_desc(const void* activations_ptr,
  ...
  // = row * N + col`).
  //
  // One TMA with `boxDim = (128, T_TILE)` fetches exactly one 1024-B
  // SWZ128 atom (T_TILE rows × 128 K-bytes) per K-step, where T_TILE
  // is 8 at BS<=8 and 16 at BS=16.  The TMA hardware applies the
  // 8-row × 128-byte core-matrix XOR swizzle at write time into
  ...
  //   boxDim[1]  = t_tile (8 at BS<=8, 16 at BS=16)
  //
