 ...
 * @tparam Fp8Tok            Must be T_TILE or T_TILE+1 (T_TILE = 8 for
 *                           BS<=8, 16 for BS=16).
...
__device__ __forceinline__ void moe_streaming_quantize_k128(
  ...
  static_assert(Dims::BS <= 16, "Streaming quantize supports BS<=16 (Req 4.1)");
  ...
  static_assert(Fp8Tok == MoECoreDims<Dims>::T_TILE ||
                ...
                "fp8_act must have T_TILE (legacy) or T_TILE+1 (kc-padded "
                "for bank-conflict avoidance) token rows; only the first "
                "T_TILE are written, the extra row — when present — is "
                "unused padding that breaks the 128-byte kc stride. "
                "T_TILE is 8 for BS<=8 and 16 for BS=16, so this resolves "
                "to 8/9 on the BS8 path and 16/17 on the BS16 path.");
...
__device__ inline void routing_phase_quantize(
  ...
  static_assert(Dims::BS <= 16,
                "routing_phase_quantize supports BS<=16 (Req 4.1)");
