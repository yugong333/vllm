...
/**
 * @brief wgmma.mma_async m64n16k32 fp8×fp8 → fp32 with SHM A and SHM B.
 *
 ...
 *
 * PTX form (from CUTLASS mma_sm90_gmma.hpp, MMA_64x16x32_F32E4M3E4M3_SS_TN):
 *   {
 *     .reg .pred p;
 *     setp.ne.b32 p, <scale_D_reg>, 0;
 *     wgmma.mma_async.sync.aligned.m64n16k32.f32.e4m3.e4m3
 *         {d0, d1, d2, d3, d4, d5, d6, d7}, desc_a, desc_b, p,
 *         <scaleA>, <scaleB>;
 *   }
 *   where scaleA / scaleB are immediates (+1 or -1).
 *
 ...
 *   scale_D = 1  (accumulate: D = A·B + D)
 *   scaleA  = 1, scaleB = 1
 * to match wgmma_m64n8k32_e4m3_e4m3_f32 — the only modes our kernel uses.
 *
 * @param desc_a   Matrix descriptor for operand A (weights, M×K)
 * @param desc_b   Matrix descriptor for operand B (activations, K×N=16)
 * @param d0..d7   8 fp32 accumulator registers (read-modify-write)
 *
 * Validates: Req 7.1 (single instruction, 8 fp32 accumulator regs/thread,
 * no spill).
 */
