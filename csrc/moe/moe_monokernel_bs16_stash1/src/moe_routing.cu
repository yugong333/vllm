 ...
 */
// ── Per-token top-K body (extracted for BS16 multi-pass reuse) ───────────
//
// Computes the top-`top_k` routing for a SINGLE token `token` and writes
// `topk_ids_flat[token*MAX_TOPK + k]` / `topk_weights_flat[...]`.  Must be
// called by all 32 lanes of one calc warp (it does warp-cooperative
// reductions); `token` is uniform across the warp.
//
// Factored out of `topK_BS8` so the BS=16 path can invoke it twice per
// calc warp (tokens `warp_idx` and `warp_idx + CALC_WARP_COUNT`) without
// duplicating the body.  `__forceinline__` guarantees the BS8 caller
// inlines it back to the exact pre-factor instruction stream, so the BS8
// SASS stays byte-identical (Req 13.2) — verified against
...
__device__ static __forceinline__ void topK_one_token(
  ...
  for (uint32_t i = 0; i < MAX_PER_THREAD; ++i) {
    ...
    scores[i] = (float)router_logits[token * Dims::NUM_EXPERTS + idx];
    expert_id[i] = idx;
  }
  ...
  #pragma unroll
      ...
      // Tie-break toward the smallest expert id (see fast-path comment).
      uint32_t tie_cand = (max_val == warp_max) ? max_expert : 0xFFFFFFFFu;
  #pragma unroll
      for (int off = 16; off >= 1; off /= 2)
        tie_cand =
            min(tie_cand, __shfl_xor_sync(0xFFFFFFFFU, tie_cand, off, 32));
      uint32_t winning_expert = tie_cand;  // warp-uniform
      float winning_weight = warp_max;     // tied lanes share the score
      if (tid == 0) {
        shmem->topk_ids_flat[token * MAX_TOPK + k] = (uint16_t)winning_expert;
        shmem->topk_weights_flat[token * MAX_TOPK + k] = winning_weight;
      }
  #pragma unroll
      for (uint32_t i = 0; i < MAX_PER_THREAD; i++) {
  ...
  #pragma unroll
    ...
    float warp_max = warp_reduce_max_float(max_val);
    // Tie-break by the SMALLEST expert id among the lanes holding the max
    // score (matches torch.topk / Triton).  The expert→lane layout is
    // strided (`expert_id = i*32 + tid`), so the lowest LANE (what the old
    // `warp_who_has` returned) is NOT the smallest expert id — on a score
    // tie at the selection boundary that picked the wrong expert and made
    // the monokernel's routing diverge from the reference (hurting
    // accuracy + spec-decode acceptance).  A warp min-reduction over the
    // candidate expert ids resolves the tie deterministically toward the
    // smaller index.
    uint32_t tie_cand = (max_val == warp_max) ? max_expert : 0xFFFFFFFFu;
  #pragma unroll
    for (int off = 16; off >= 1; off /= 2)
      tie_cand = min(tie_cand, __shfl_xor_sync(0xFFFFFFFFU, tie_cand, off, 32));
    uint32_t winning_expert = tie_cand;  // warp-uniform
    float winning_score = warp_max;      // all tied lanes share the score
...
__device__ void topK_BS8(uint32_t top_k, ScoringFunc scoring_func,
  ...
  static_assert(Dims::BS <= 16, "topK_BS8 supports BS<=16 (Req 2.1)");
  ...
  // ── Token → calc-warp mapping ──────────────────────────────────────────
  // BS<=8: each calc warp routes exactly one token (`tok == warp_idx`);
  // there are `CALC_WARP_COUNT = 8` calc warps, enough for all <=8 tokens.
  // Padding warps (`warp_idx >= num_tokens`) do nothing.
  //
  // BS=16: there are still only 8 calc warps but up to 16 real tokens, so
  // each calc warp must route TWO tokens — `warp_idx` (pass 0) and
  // `warp_idx + CALC_WARP_COUNT` (= `warp_idx + 8`, pass 1).  Without the
  // second pass, `topk_ids_flat[64..127]` (tokens 8..15) would stay
  // uninitialized; `prepare_moe_topk_BS8`'s Phase-C ranking then reads
  // those garbage ids and indexes `expert_slot_start[garbage]` out of
  // bounds (the illegal `LDS.U16` fault).  The second pass is gated under
  // `if constexpr (Dims::BS == 16)` so the BS8 instruction stream is
  // unchanged (Req 13.2).
...
__device__ void prepare_moe_topk_BS8(uint32_t batch_size, uint32_t top_k,
  ...
  // (BS=16: 4 pairs/lane).  Declared at function scope so Phase C can
  // reuse them; on BS=8 (MAX_PAIRS == 64) they are unused and DCE'd, so
  // no new SASS is emitted (Req 13.2).
  ...
  // chunk-2 / chunk-3 eid caches.  Declared at function scope (reused in
  // Phase A tally AND Phase C ranking) but the SHM loads are gated under
  // `MAX_PAIRS > 64u` so BS=8 issues no load and the variables fold to a
  // dead sentinel that ptxas eliminates — keeping BS=8 SASS bit-identical
  // (Req 13.2).
  ...
  *reinterpret_cast<uint4*>(&tma_shm->expert_slot_start[tid * BLK]) =
  ...
  // Chunk-2 / chunk-3 ranking (BS=16 only, MAX_PAIRS > 64).  Each pair's
  // global rank within its expert's contiguous slab is
  //   (intra-chunk rank within its own chunk)
  // + (count of same-eid pairs in all LOWER-indexed chunks).
  // Chunk ordering is chunk0 < chunk1 < chunk2 < chunk3 (pair indices
  // tid, tid+32, tid+64, tid+96), so:
  //   chunk-2 carry = carry(from chunk0) + carry(from chunk1)
  //   chunk-3 carry = carry(from chunk0) + carry(from chunk1)
  //                 + carry(from chunk2)
  // Each carry(from chunkJ) for a lane's eidX is the number of warp lanes
  // whose chunkJ eid equals eidX — computed with the same 32-round
  // shfl+ballot+popc sweep used for rank1_carry.
  ...
  if constexpr (MAX_PAIRS > 64u) {
    if (p2 < n_pairs && eid2 != 0xFFFF) {
      const uint8_t slot2 = static_cast<uint8_t>(
          tma_shm->expert_slot_start[eid2] + rank2_intra + rank2_carry);
    ...
    if (p3 < n_pairs && eid3 != 0xFFFF) {
      const uint8_t slot3 = static_cast<uint8_t>(
          tma_shm->expert_slot_start[eid3] + rank3_intra + rank3_carry);
