...
/**
 * @brief Macro that expands to a kernel call wrapper for moe_kernel with
 * specified @p dims
 *
 * moe_kernel() needs to be instantiated with different parameters. In the past,
 * dispatching to the different kernels on the device or on the host within C++
 * code generated problems in vLLM and PyTorch. So these wrappers are to move
 * this dispatch to Python. For each instantiated kernel, we generate one host
 * invocation function.
 */
#define MOEMONOKERNEL_WRAPPER_IMPLEMENTATION(name, dims)                       \
  void name(const torch::Tensor& activations_in,                               \
    ...
    CUDA_CHECK(cudaFuncSetAttribute(                                           \
        moe_kernel<dims>, cudaFuncAttributeMaxDynamicSharedMemorySize,         \
        shmem_size));                                                          \
    CUDA_CHECK(cudaLaunchCooperativeKernel(                                    \
        moe_kernel<dims>, dims::KernelConfig::GRID_SIZE,                       \
        dims::KernelConfig::BLOCK_SIZE, kernel_args, shmem_size, stream));     \
  }

MOEMONOKERNEL_WRAPPER_IMPLEMENTATION(moe_monokernel_BS8_E16_TP8_impl,
                                     moe_monokernel::Dims_BS8_E16_TP8)
MOEMONOKERNEL_WRAPPER_IMPLEMENTATION(moe_monokernel_BS64_E16_TP8_impl,
                                     moe_monokernel::Dims_BS64_E16_TP8)

MOEMONOKERNEL_WRAPPER_IMPLEMENTATION(moe_monokernel_BS8_E128_TP8_impl,
                                     moe_monokernel::Dims_BS8_E128_TP8)
MOEMONOKERNEL_WRAPPER_IMPLEMENTATION(moe_monokernel_BS64_E128_TP8_impl,
                                     moe_monokernel::Dims_BS64_E128_TP8)
...
// Top-K variants for E128 (Qwen3 Coder uses 128 experts with top_k=8)
MOEMONOKERNEL_TOPK_WRAPPER_IMPLEMENTATION(moe_monokernel_topk_BS8_E128_TP8_impl,
                                          moe_monokernel::Dims_BS8_E128_TP8)
MOEMONOKERNEL_TOPK_WRAPPER_IMPLEMENTATION(
    moe_monokernel_topk_BS64_E128_TP8_impl, moe_monokernel::Dims_BS64_E128_TP8)

// Top-K variants for E16
MOEMONOKERNEL_TOPK_WRAPPER_IMPLEMENTATION(moe_monokernel_topk_BS8_E16_TP8_impl,
                                          moe_monokernel::Dims_BS8_E16_TP8)
MOEMONOKERNEL_TOPK_WRAPPER_IMPLEMENTATION(moe_monokernel_topk_BS64_E16_TP8_impl,
                                          moe_monokernel::Dims_BS64_E16_TP8)

// Top-K variants for Qwen3-Coder-30B-A3B (E=128, K=2048, N=768, TP=1)
MOEMONOKERNEL_TOPK_WRAPPER_IMPLEMENTATION(
