...
static_assert(false,
   ...
   * This operation is collective and needs to be called by all threads within a
   * warp.
   *
   */
   ...
   *
   * Computes a prefix sum over 16 uint8. First element is the least significant
   * byte.
   */
  ...
  /**
   * @brief Prepares the MoE computation for batch size 8 and 16 experts for the
   * matrix multiply functions.
   *
   * Reads @c shm->topk_ids and sets up the necessary data structures and state
   * for @c moe_up_projection and @c moe_down_projection . Specifically, it
   * fills:
   * - @c shm->expert_mask
   * - @c shm->expert_ids
   * - @c shm->expert_count
   *
   * The computation is done redundantly on each CUDA block such that the result
   * can be stored in shared memory.
   *
   * @param batch_size The batch size (active elements in the topk_ids array)
   * @param shm Pointer to shared memory struct to read inputs from and write
   * outputs to.
   *
   */
  template <typename Dims>
  __device__ static void prepare_moe_BS8_E16(std::uint32_t batch_size,
                                             MoE_SHM<Dims>* __restrict__ shm) {
    static_assert(Dims::NUM_EXPERTS <= 16,
                  "This function is only for up to 16 experts");
    static_assert(Dims::BS <= 8,
                  "This function is only for up to batch size 8");
    assert(batch_size <= 8);

    std::uint64_t packed_unique_topkids = 0;

    // Initialize the SHM in every SM.
    // One thread in each warp does this so that we do not need a block-wide
    // synchronization before accessing the first expert
    if (threadIdx.x % 32 == 0) {
      ...
      // Extract the up to 8 unique experts from the bitfield, by finding the up
      // to 8 bits that are set to 1.
      //
      // Only the first "expert_count" of these are valid, the other eX values
      // are 0xFFFFFFFF
      ...
      packed_unique_topkids =
          lo_packed_unique_topkids | ((uint64_t)hi_packed_unique_topkids << 32);
  ...
  /**
   * @brief Prepares the MoE computation for batch size 8 and 128 experts for
   * the matrix multiply functions.
   *
   * Reads @c shm->topk_ids and sets up the necessary data structures and state
   * for @c moe_up_projection and @c moe_down_projection . Specifically, it
   * fills:
   ...
   */
  template <typename Dims>
  __device__ static void prepare_moe_BS8_E128(std::uint32_t batch_size,
                                              MoE_SHM<Dims>* __restrict__ shm) {
    static_assert(Dims::NUM_EXPERTS <= 128,
                  "This function is only for up to 128 experts");
    static_assert(Dims::BS <= 8,
                  "This function is only for up to batch size 8");
    assert(batch_size <= 8);
    ...
    // Initialize the SHM in every SM.
    // One thread in each warp does this so that we do not need a block-wide
    // synchronization before accessing the first expert For explanation of the
    // method, see prepare_moe_BS8_E16
    if (threadIdx.x % 32 == 0) {
      uint64_t packed_topkids = *(uint64_t*)shm->topk_ids;
      ...
      // If expert_count == 7, b1 is copied into b0 again before the last
      // __ffsll
  ...
  /**
   * @brief Prepares the MoE computation for batch size 64 and 16 experts for
   * the matrix multiply functions.
   *
   * Reads @c shm->topk_ids and sets up the necessary data structures and state
   * for @c moe_up_projection and @c moe_down_projection . Specifically, it
   ...
   */
  template <typename Dims>
  __device__ static void prepare_moe_BS64_E16(std::uint32_t batch_size,
                                              MoE_SHM<Dims>* __restrict__ shm) {
    static_assert(Dims::NUM_EXPERTS <= 16,
                  "This function is only for up to 16 experts");
    static_assert(Dims::BS <= 64,
                  "This function is only for up to batch size 64");
    ...
    const uint8x16_t token_counts =
        allreduce_sum_across_warp(token_count_local);
    const uint8x16_t token_prefixes = prefix_sum_over_bytes(token_counts);

    // Every thread is now responsible for one expert.
    static_assert(
        Dims::NUM_EXPERTS <=
        warp_size);  // Need at least one thread per expert in each warp
    ...
    // "inverse_permute_last" is the source lane for compacting "token_count".
    uint32_t inverse_permute_last = __fns(nonzero_mask, 0, thread_idx + 1);
    uint32_t inverse_permute_first =
        __fns(nonzero_mask, 0, thread_idx);  // Index of token start is the
                                             // previous thread's end index.
    uint32_t token_start =
        __shfl_sync(FULL_MASK, token_prefix, inverse_permute_first);
    uint32_t token_end =
        __shfl_sync(FULL_MASK, token_prefix, inverse_permute_last);
    ...
    if (warp_idx == 0) {
      ...
      if (thread_idx < num_nonzero_elements) {
        shm->experts[thread_idx].id = inverse_permute_last;
    ...
    //
    if constexpr (Dims::BS <= warp_size) {  // Can be handled completely within
                                            // a single warp
      uint32_t eid;
      ...
      uint32_t mask_of_threads_with_same_prefix =
          __match_any_sync(FULL_MASK, sorting_prefix);
    ...
    } else if constexpr (Dims::BS <=
                         2 * warp_size) {  // Can be handled by two warps
      if (warp_idx < 2) {
        uint32_t eid;
        ...
        // For threads with the same sorting_prefix:
        // We define here that their threadIdx determines the relative token
        // order Tokens that are processed by threads from the first warp are
        // sorted upwards, otherwise downwards.
        uint32_t mask_of_threads_with_same_prefix =
            __match_any_sync(FULL_MASK, sorting_prefix);
        // Relative order == number of one bits in mask before its position
        uint32_t lower_idx_mask = (1 << thread_idx) - 1;
        uint32_t rank =
            __popc(mask_of_threads_with_same_prefix & lower_idx_mask);
        ...
        if (thread_idx + warp_size * warp_idx < batch_size) {
          shm->token_indexes[target_lane_id] =
              thread_idx + warp_idx * warp_size;
        }
      }
    }
  }
  ...
  /**
   * @brief Prepares the MoE computation for batch size 64 and 128 experts for
   * the matrix multiply functions.
   *
   * Reads @c shm->topk_ids and sets up the necessary data structures and state
   * for @c moe_up_projection and @c moe_down_projection . Specifically, it
   ...
   * can be stored in shared memory.
   *
   * This function should support any number of experts, but is only tested for
   * up to 128.
   *
   * @param batch_size The batch size (active elements in the topk_ids array)
   * @param shm Pointer to shared memory struct to read inputs from and write
   ...
   */
  template <typename Dims>
  __device__ static void prepare_moe_BSx_Ey(std::uint32_t batch_size,
    ...
    if (threadIdx.x < CoreDims::THREADS_PER_WARP) {
      ...
      // count inputs
      for (unsigned i = threadIdx.x; i < batch_size;
  ...
  /**
   * @brief Dispatch function for @c prepare_moe_BS8_E16 and @c
   * prepare_moe_BS8_E128 .
   */
  template <typename Dims>
  __device__ __forceinline__ void prepare_moe_BS8(
      std::uint32_t batch_size, MoE_SHM<Dims>* __restrict__ shm) {
    static_assert(Dims::NUM_EXPERTS <= 128,
                  "This function is only for up to 128 experts");
    static_assert(Dims::BS <= 8,
                  "This function is only for up to batch size 8");
   ...
   *  - @c shmem->token_indexes_topk — original token index per sorted slot
   *  - @c shmem->token_weights      — routing weight per sorted slot
   *
   * The virtual batch size is @p num_tokens * @p top_k.  Each virtual row
   * @c i corresponds to token @c (i / top_k), k-slot @c (i % top_k), with
   * expert id @c topk_ids_flat[i] and weight @c topk_weights_flat[i].
   *
   * The sort is a radix sort identical to @c prepare_moe_BSx_Ey, but operating
   * on the flat array of size @c num_tokens * top_k.
   *
  ...
  __device__ static void prepare_moe_topk_BSx_Ey(
    ...
    // Reuse the SortData union — same radix sort, larger virtual batch.
    ...
    if (threadIdx.x < CoreDims::THREADS_PER_WARP) {
      ...
      // count: iterate over all virtual rows using strided layout
      // topk_ids_flat is [token * MAX_TOPK + k] with valid entries for k <
      // top_k
      ...
      // prefix-sum per expert (same as prepare_moe_BSx_Ey)
      for (unsigned e = threadIdx.x; e < Dims::NUM_EXPERTS;
           e += CoreDims::THREADS_PER_WARP) {
        std::uint32_t sum = 0;
        for (unsigned i = 0; i < CoreDims::THREADS_PER_WARP; ++i) {
      ...
      // write token_indexes_topk and token_weights using strided layout
      for (unsigned i = threadIdx.x; i < virtual_batch;
           i += CoreDims::THREADS_PER_WARP) {
        unsigned tok = i / top_k;
        unsigned k = i % top_k;
        ...
        // original token index = tok (not virtual row / top_k)
        shmem->token_indexes_topk[index] = (std::uint16_t)tok;
        shmem->token_weights[index] =
