...
// Stable-ABI build (TORCH_TARGET_VERSION defined): full-libtorch headers are
// forbidden, so error reporting uses STD_TORCH_CHECK from the C-shim helpers
// instead of TORCH_CHECK.  This TU only builds CUtensorMap descriptors from
// raw pointers + dims — it never touches a Tensor.
...
CUtensorMap create_up_weight_tma_desc(const void* weights_ptr,
                                      ...
                                      uint32_t K) {
  // SWIZZLE_128B up-weight descriptor over the `[E, 2*N, K]` fp8 tensor.
  // The descriptor is layout-agnostic (outer dim = E*2*N, boxDim = 128
  // rows × 128 K); only the row COORDINATE the kernel passes differs
  // between the two consumers:
  //
  //   * 35B (interleaved, `tma_load_up_wgmma_tile`): the Python
  //     `interleave_for_tma_wgmma_up[_v2]` pre-packs each 128-row WGMMA
  //     A-tile (gate+up stripes) into a contiguous GM block, fetched at
  //     coord1 = expert_id * 2 * N + 2 * base_row_up.
  //
  //   * 122B (two-TMA RAW, `tma_load_up_wgmma_tile_raw`): NO pre-interleave.
  //     The kernel issues two 128-row TMAs from the unmodified tensor —
  //     gate at coord1 = e*2*N + base_row_up, up at e*2*N + N + base_row_up.
  //
  // Either way a single `boxDim = (128, 128)` TMA fetches a 128×128 fp8 A
  // operand in one issue; the SHM bytes after the TMA's SWZ128 XOR match
  // the canonical Major::K B128 layout expected by the WGMMA A-descriptor
