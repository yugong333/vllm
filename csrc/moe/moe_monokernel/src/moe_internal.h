...
// ── Up-proj weight-TMA lookahead depth opt-in detection ─────────────────
// `Dims::KernelConfig::UP_W_SLOTS` is optional; it is the number of
// physical `bar_w` / `w_wgmma` double-buffer slots used by the up-proj
// weight-TMA prefetch pipeline.  When absent, defaults to the historical
// per-shape value: 4 for the single-atom (UP_COL_HALVES==1, 35B) shape,
// 2 for the two-atom (UP_COL_HALVES==2, 122B) shape — these were the
// hand-tuned depths that the two forks shipped with.
//
// The PREFETCH DISTANCE (how many outer-K iters ahead the launcher arms)
// is DERIVED, not a separate knob: `UP_ARM_DISTANCE = max(1, SLOTS - 2)`.
// This reproduces both shipped kernels exactly (4 slots → arm s+2;
// 2 slots → arm s+1) and keeps the wraparound-safety rule
// `SLOTS >= ARM_DISTANCE + 2` satisfied by construction (a slot is not
// re-armed until its previous consumer wait has completed).
//
// Constraints (enforced by static_asserts at the use site):
//   * 2 <= UP_W_SLOTS, and UP_W_SLOTS is a power of two so the `s % S`
//     slot index can use a cheap mask (`s & (S-1)`).
//   * K_TILES_UP % UP_W_SLOTS == 0 so the cross-expert stitch lands the
//     next expert's iters [0, ARM_DISTANCE) on slots [0, ARM_DISTANCE),
//     keeping the per-slot arm/wait parity balanced across experts.
//   * Per-slot SHM cost = UP_COL_HALVES * K_STEP_UP * 128 B; the union
//     with `w_down_wgmma` plus the <=224 KB budget bounds the product
//     UP_W_SLOTS * UP_COL_HALVES * K_STEP_UP.
// Defined below `down_col_tile` so the per-shape default can derive
// UP_COL_HALVES = (2*N * DOWN_COL_TILE) / (128 * HIDDEN_STATES).

// ── Down-proj output col-tile opt-in detection ──────────────────────────
// `Dims::KernelConfig::DOWN_COL_TILE` is optional.  When absent we fall
// back to the legacy rule `(use_tma && BS<=8) ? 256 : 128` so existing
// variants keep their behaviour byte-for-byte.  Variants that need a
// different output col-tile per block (e.g. the 122B shape, which needs
// 384 = 3 × 128 to keep DOWN_GROUPS == 16 == UP_GROUPS) set it
// explicitly.  Must be a multiple of 128 (the SWZ128 atom M-width) and a
// divisor of HIDDEN_STATES; `DOWN_COL_HALVES = DOWN_COL_TILE / 128` ∈
// {1, 2, 3} is supported by the WGMMA down-proj M-axis loop.
template <typename Dims>
...
// ── Up-proj weight-TMA lookahead depth (UP_W_SLOTS) opt-in detection ─────
// See the design note above `down_col_tile`.  Placed here so the per-shape
// default can derive UP_COL_HALVES from `down_col_tile<D>`.
template <typename Dims>
struct up_w_slots {
 private:
  template <typename D>
  ...
  static constexpr std::uint32_t test(...) {
    // UP_COL_HALVES = (2*N * DOWN_COL_TILE) / (128 * HIDDEN_STATES),
    // floored at 1 (mirrors MoECoreDims::UP_COL_HALVES).  Historical
    // per-shape default: 4 slots for the single-atom shape, 2 for the
    // two-atom shape.
...
struct MoEGemmSpec {
  ...
  // Activation-quantization block size along the intermediate (N) axis.
  // The up-proj epilogue computes one fp8 scale per (block, token), where
  // a "block" is the set of intermediate features one up-block owns.
  //
  //   * 35B: each up-block owns 64 features → DOWN_ACT_BLOCK_SIZE = 64.
  //   * 122B: each up-block owns 128 features (UP_COL_HALVES = 2) →
  //     DOWN_ACT_BLOCK_SIZE = 128 (point 3).
  //
  // Derived as `(2*N / W_UP_TILE_EFFECTIVE) ... ` would create a forward
  // dependency on MoECoreDims, so we compute it directly from the same
  // inputs: the per-up-block feature count equals
  // `UP_COL_HALVES * 128 / 2` (gate features only; up features pair 1:1).
  // `UP_COL_HALVES = (2*N * DOWN_COL_TILE) / (128 * HIDDEN_STATES)` — see
  // the MoECoreDims::UP_COL_HALVES comment for the derivation.  For the
  // two instantiated shapes this evaluates to 64 (35B) / 128 (122B).
  static constexpr uint32_t UP_COL_HALVES_LOCAL =
      ((2u * Dims::N * down_col_tile<Dims>::value) /
           (128u * Dims::HIDDEN_STATES) >
       0u)
          ? (2u * Dims::N * down_col_tile<Dims>::value) /
                (128u * Dims::HIDDEN_STATES)
          : 1u;
  ...
  // sizes
  // 4th template param = max NUM_EXPERTS; sizes E-indexed SHM/scratchpad
  // arrays (expert_slot[E][2], expert_slot_start[E], experts[E], ...) and is
  // the ceiling for the `NUM_EXPERTS <= Dims_Max::NUM_EXPERTS` static_assert.
  // Bumped 256 -> 512 to admit the E-sweep shapes (E up to 512) in shapes.json.
  #if USE_SMALL_SETUP
// SHM limits batch size to ~2k
using Dims_Max = MoEDimensions<1024, 256, 1024, 512>;
  #else
using Dims_Max = MoEDimensions<1024, 1024, 5120, 512>;
  #endif
...
struct MoECoreDims {
  ...
  // ── Up-proj weight-TMA lookahead pipeline (UP_W_SLOTS tunable) ───────
  // Number of physical `bar_w` / `w_wgmma` double-buffer slots and the
  // DERIVED prefetch distance.  See the `up_w_slots` design note.  The
  // launcher arms slot `(s + UP_ARM_DISTANCE) % UP_W_SLOTS` for logical
  // outer-K iter `s + UP_ARM_DISTANCE`; the consumer waits slot
  // `s % UP_W_SLOTS`.  UP_ARM_DISTANCE = max(1, SLOTS - 2) keeps the
  // re-arm of a slot strictly after its previous consumer wait
  // (wraparound-safety needs SLOTS >= ARM_DISTANCE + 2; equality at
  // SLOTS=2→A=1 and the slack grows for deeper pipelines).
  ...
  // ── Up-proj output col-halves (M-axis 128-row atoms per block) ───────
  // Each up-block owns `UP_COL_HALVES` stacked 128-row WGMMA M-atoms.
  // Chosen so UP_GROUPS == DOWN_GROUPS (the Phase-2b Site-#2
  // expert_barrier requires the producer set == consumer set):
  //
  //   UP_GROUPS  = GRID_SIZE / (2*N / (128 * UP_COL_HALVES))
  //   DOWN_GROUPS= GRID_SIZE / (HIDDEN_STATES / DOWN_COL_TILE)
  //   UP_GROUPS == DOWN_GROUPS
  //     ⇒ UP_COL_HALVES = (2*N * DOWN_COL_TILE) / (128 * HIDDEN_STATES).
  //
  // 35B (N=512, DOWN_COL_TILE=256, HIDDEN=2048): = 1 (one atom/block,
  //   byte-identical to the pre-parameterization kernel).
  // 122B (N=1024, DOWN_COL_TILE=384, HIDDEN=3072): = 2 (two atoms/block;
  //   128 gate features + 128 up features → 128 intermediate features).
  // Floored at 1 so generic placeholder Dims (e.g. Dims_Max, used only
  // for SHM-size bookkeeping and never launched) don't divide-by-zero in
  // the derived constants below; the WGMMA shapes evaluate to 1 (35B) /
  // 2 (122B) before the floor matters.
  ...
  // Per-up-block intermediate-feature count = activation-quantization
  // block size along N (point 3).  One up-block owns `UP_COL_HALVES`
  // 128-row atoms = `UP_COL_HALVES * 64` gate features (each atom packs
  // 64 gate + 64 up rows).  35B → 64, 122B → 128.
  static constexpr std::uint32_t DOWN_ACT_BLOCK_SIZE = UP_COL_HALVES * 64u;
  // Number of activation-scale blocks along the down-proj reduction
  // dim N (= one per up-block).  35B → 512/64 = 8, 122B → 1024/128 = 8.
  static constexpr std::uint32_t DOWN_ACT_HALVES =
      Dims::N / DOWN_ACT_BLOCK_SIZE;

  // Effective M (row-tile) size of one block's up-proj work —
  // UP_COL_HALVES × 128 for the WGMMA path, 16 for the scalar path.
  // Used to compute UP_GRID = 2*N / W_UP_TILE_EFFECTIVE.
...
struct MoE_SHM {
  ...
  union U {
    ...
    struct TinyDataWGMMA_TMA {
      ...
      // Up weight tile stacks BOTH the K substeps AND the UP_COL_HALVES
      // 128-row M-atoms along the M axis: substep `kk`, half `h` →
      // rows [(kk*UP_COL_HALVES + h)*128 .. +128).  35B (1 substep, 1
      // half) → 128 rows; 122B (1 substep, 2 halves) → 256 rows.
      ...
      // Up-proj weight-TMA lookahead depth (see w_wgmma below).  Declared
      // at struct scope (not inside the anonymous union, where static
      // members are illegal) so the array bound is a constant expression.
      ...
      union {
        ...
        // Up-proj weight slots.  Slot count is the weight-TMA lookahead
        // depth: 4 for the 35B shape (UP_COL_HALVES=1, each slot 16 KB →
        // 64 KB), 2 for the 122B shape (UP_COL_HALVES=2, each slot 32 KB →
      ...
      // One fp32 activation scale per (up-block, token) along the full
      // down-proj reduction dim N.  Count = N / DOWN_ACT_BLOCK_SIZE
      // (= number of up-blocks spanning N).  35B: 512/64 = 8; 122B:
      // 1024/128 = 8 (the count is 8 for both — only the per-block K
      // width differs).
      ...
      // [DOWN_COL_HALVES][WG0/WG1][N/128 col-blocks].  First dim = the
      // number of 128-row M-atoms one down-block owns (DOWN_COL_TILE/128):
      // 2 for 35B, 3 for 122B.
      ...
      union {
        ...
        // to Phase 3.1.
        // Sized `UP_COL_HALVES * 128` rows so each of the up-block's
        // M-atoms gets its own 128-row sub-region: atom `h` uses rows
        // [h*128 .. h*128+128).  Within an atom only rows [0..31]
        // (WG0) and [64..95] (WG1) carry the 64 post-SiLU features; the
        // gaps exist because the layout aliases V1's wgmma_out 128-row
        // extent.  35B → [128][9]; 122B → [256][9] (still well below the
        // `down_out` union dominator).
      ...
      // Up-proj weight-TMA mbarriers: one per lookahead slot
      // (`UP_W_SLOTS`, tunable; 4 for the 35B shape, 2 for 122B, or any
      // power-of-two >= 2 set via KernelConfig).  The down-proj reuses
      // `bar_w[0..1]` (a 2-slot ping-pong), so the array must be at
      // least 2 wide — guaranteed by the `UP_W_SLOTS >= 2` static_assert.
      alignas(16) uint64_t bar_w[UP_W_SLOTS];  // 8 B per slot
