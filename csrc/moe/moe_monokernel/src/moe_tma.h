...
/**
 * @brief RAW (non-interleaved) 128×128 up/gate weight TMA loader.
 *
 * Fetches one contiguous 128-row × 128-K fp8 tile straight from the
 * unmodified `[E, 2*N, K]` weight tensor — NO Python gate/up pre-interleave
 * is applied.  Used by the 122B two-TMA up-projection: each WGMMA M-atom
 * is one half (gate OR up), so the kernel issues two of these per K-substep
 * (one at the gate rows, one at the up rows) instead of a single interleaved
 * 128-row tile.
 *
 * GM row layout (raw): for expert `e`, rows `[0, N)` are gate, rows
 * `[N, 2*N)` are up.  A 128-row tile of gate starting at original gate row
 * `row_in_half` lives at flattened outer coordinate
 *   `e * 2 * N + (is_up ? N : 0) + row_in_half`.
 *
 * Caller contract is identical to `tma_load_up_wgmma_tile` (one launcher
 * thread, bar pre-armed once with the total tx_bytes, 1024-B aligned dest,
 * descriptor from `create_up_weight_tma_desc`).  Because the descriptor's
 * outer dimension is `E * 2 * N` either way, the SAME descriptor is reused;
 * only the row coordinate differs (no `2*` interleave skip).
 *
 *   - Valid inputs: `0 ≤ expert_id < num_experts`,
 *     `0 ≤ row_in_half + 128 ≤ N`, `is_up ∈ {0, 1}`,
 *     `0 ≤ k_start` with `k_start + 128 ≤ K`.
 */
__device__ __forceinline__ void tma_load_up_wgmma_tile_raw(
  ...
  // Raw `[E, 2*N, K]` view: gate at rows [0,N), up at rows [N,2N) per expert.
