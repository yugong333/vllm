 ...
 */
// [TASK12 PROBE TOGGLE] Define to inject the A' raw-32/32 cross-warpgroup
// exchange into the UCH==1 up-proj epilogue (numerically inert; measures the
// exchange's wall-clock cost vs the interleaved baseline).  Remove after the
// go/no-go measurement.
#define MONO_PROBE_RAW32_UPPROJ 1
...
__device__ inline void moe_up_projection_BS8_allexperts_wgmma_tma(
  ...
  // Weight-TMA lookahead pipeline depth (tunable) + derived arm distance.
  // SLOTS=4 → A=2 reproduces this kernel's original 4-slot / arm-(s+2)
  ...
  // The SLOTS-deep pipeline requires K_TILES to be a multiple of SLOTS so
  // the end-of-K-loop cross-expert stitch arms the next expert's first A
  // iters into slots [0, A) — exactly the slots its pre-loop would
  // otherwise prime, and the slots its iter-0..A-1 COMPUTE waits on.  A
  // non-multiple would phase-shift the slot index across the expert
  // boundary, breaking the mbarrier parity chain.  (The authoritative
  // check is `K_TILES_UP % UP_W_SLOTS == 0` in MoECoreDims; this mirror
  // documents the requirement at the consuming kernel.)  Examples:
  // K_STEP_UP=128 → K_TILES=16; K_STEP_UP=256 → K_TILES=8; both are
  // multiples of SLOTS ∈ {2,4}.
  ...
  // Issue the K_SUBSTEPS weight TMAs for one slot of a given expert /
  // outer-K iter into `bar_w[slot]`.  Caller pre-arms `bar_w[slot]` with
  // UP_W_TX_BYTES_TOTAL.  Single-atom (UP_COL_HALVES==1) interleaved
  // layout: the Python pre-interleave packs the full 128-row WGMMA A-tile,
  // so one `tma_load_up_wgmma_tile` per K-substep fetches it.
  ...
  //
  // Generalized SLOTS-deep lookahead (A = UP_ARM_DISTANCE, S = SLOTS): at
  // iter s the launcher arms slot `(s+A) % S` for logical iter s+A; the
  // pre-loop primes the first A slots (iters 0..A-1).  For SLOTS=4 (A=2)
  // this primes slots 0 and 1 — byte-identical to this kernel's original
  // hand-written dual pre-arm.  The bf16-input TMA + `bar_a` arm were
  // removed (Req 3.7); Phase 1/2 populate `fp8_act_full` once per kernel.
  //
  // Compiled out under MONO_PROFILE_SKIP_PREFETCH_UP; the matching calc-
  // warp wait inside the K-loop is also compiled out so there is no
  ...
  #pragma unroll
    ...
    for (uint32_t s = 0; s < K_TILES; ++s) {
      // SLOTS-deep lookahead: calc waits `bar_w[s % S]`; launcher arms
      // `bar_w[(s+A) % S]` for logical iter s+A.  For SLOTS=4 (A=2) the
      // arm lands 2 iters before the matching consumer wait — identical
      // to this kernel's original 4-deep pipeline.  S is a power of two
  ...
  #ifndef MONO_PROFILE_SKIP_PREFETCH_UP
        // SLOTS-deep lookahead: at iter `s` arm `bar_w[(s+A) % S]` for the
        // logical iter `s + A` weight tile (A = ARM_DISTANCE).  Target:
        //   * intra-expert if `s + A < K_TILES` → CURRENT expert's
        //     iter (s+A);
        //   * else cross-expert → NEXT expert's iter `s + A - K_TILES`
        //     (in [0, A); the last A iters stitch the next expert's first
        //     A iters into slots [0, A), matching its pre-armed pipeline).
        // For SLOTS=4 (A=2) the intra-expert branch is the old case (A)
        // and the cross-expert branch covers the old cases (B)/(C)
        // (iter-0 at s==K_TILES-2, iter-1 at s==K_TILES-1).
        // `K_TILES % SLOTS == 0` guarantees the stitch slot alignment.
        const uint32_t target_iter = s + ARM_DISTANCE;
        if (target_iter < K_TILES) {
          // Intra-expert fetch of the (s+A)-th tile.
          mbarrier_arrive_expect_tx(&shm->bar_w[arm_slot],
                                    /*tx_bytes=*/UP_W_TX_BYTES_TOTAL);
          tma_slot(arm_slot, id, target_iter);
        } else if (has_next_e) {
          // Cross-expert stitch: next expert's iter (target_iter-K_TILES).
          mbarrier_arrive_expect_tx(&shm->bar_w[arm_slot],
                                    /*tx_bytes=*/UP_W_TX_BYTES_TOTAL);
          tma_slot(arm_slot, next_id, target_iter - K_TILES);
        }
          // Else: last expert, last A iters — leave barriers idle.
  #endif
      }
  ...
  #ifdef MONO_PROBE_RAW32_UPPROJ
    // ── A' raw-32/32 TIMING PROBE (numerically inert) ────────────────────
    //
    // Task 12 go/no-go: the A' no-interleave up-proj keeps the SAME WGMMA
    // count as the interleaved default; its ONLY new compute cost is the
    // cross-warpgroup exchange (WG0 owns the 64 gate features, WG1 the 64
    // up features for the same block, so the silu(gate)*up combine is no
    // longer register-local — WG0 must ship its gate accumulators to WG1
    // via SHM + a block barrier).  This probe injects that EXACT exchange
    // (4-float store → __syncthreads → partner 4-float load) so a
    // before/after build pair measures its wall-clock cost.  The result is
    // dropped into a volatile sink — the real register-local Pair_Layout
    // combine below still runs untouched, so accuracy is UNCHANGED and the
    // probe is a pure additive timing perturbation (slightly conservative:
    // it adds one extra block sync the real A' kernel folds into the
    // existing bottom-of-loop sync).  Defined only for the UCH==1 35B shape.
    static_assert(CoreDims::UP_COL_HALVES == 1u,
                  "MONO_PROBE_RAW32_UPPROJ targets the UCH==1 (35B) shape.");
    {
      // Reuse post_silu_scratch[row][0..3] as the gate-accumulator mailbox.
      // The real V2 calc epilogue below is the last writer of every cell the
      // PF reader consumes (rows [0..31]/[64..95], cols = tokens [0..7]), so
      // this scratch use cannot perturb the deferred writeback.
      auto& mbox = shm->partial_result.post_silu_scratch;
      if (is_wg0) {
        const uint32_t r = warp_in_wg * 32u + lane;  // 0..127 (WG0 threads)
        mbox[r][0] = final_d0;
        mbox[r][1] = final_d1;
        mbox[r][2] = final_d2;
        mbox[r][3] = final_d3;
      ...
      __syncthreads();
      if (is_wg1) {
        const uint32_t r = warp_in_wg * 32u + lane;  // partner WG0 thread
        // A' combine: WG1's up accumulators × WG0's gate (read here).
        const float g0 = mbox[r][0], g1 = mbox[r][1];
        const float u0 = final_d2, u1 = final_d3;
        volatile float sink =
            __fdividef(u0 * g0, 1.0f + __expf(-g0)) +
            __fdividef(u1 * g1, 1.0f + __expf(-g1));
        (void)sink;
      }
    }
  #endif
...
__device__ inline void moe_up_projection_BS8_122B_wgmma_tma(
  ...
  // Weight-TMA lookahead pipeline depth (tunable) + derived arm distance.
  // SLOTS=2 → A=1 reproduces the original 2-slot ping-pong byte-for-byte.
  constexpr uint32_t SLOTS = CoreDims::UP_W_SLOTS;
  constexpr uint32_t ARM_DISTANCE = CoreDims::UP_ARM_DISTANCE;
  ...
  // ── Pre-loop: prime slots [0, ARM_DISTANCE) for expert_start ────────────
  //
  // Generalized SLOTS-deep lookahead: at iter s the launcher arms slot
  // `(s + A) % S` for logical iter s+A (A = UP_ARM_DISTANCE, S = SLOTS).
  // The pre-loop primes the first A slots (iters 0..A-1); slots A..S-1 and
  // every later slot are armed by the in-loop launcher below.  Priming any
  // slot the s<A launcher will also arm would double-arm it, so we prime
  // EXACTLY iters [0, A).  For SLOTS=2 (A=1) this primes slot 0 only
  // (byte-identical to the old 2-slot fork); for SLOTS=4 (A=2) it primes
  // slots 0 and 1 (matching the 35B fork's pre-loop).
  if (is_tma_launcher_thread<Dims>() && expert_start < expert_count) {
  #ifndef MONO_PROFILE_SKIP_PREFETCH_UP
    const uint32_t first_id = shmem->experts[expert_start].id;
    static_assert(K_TILES >= SLOTS && K_TILES % SLOTS == 0,
                  "up-proj SLOTS-deep pipeline requires K_TILES a multiple "
                  "of UP_W_SLOTS and >= UP_W_SLOTS so per-slot arm/wait "
                  "parity balances across experts and the cross-expert "
                  "stitch lands the next expert's iters [0,A) on slots "
                  "[0,A).");
  #pragma unroll
    for (uint32_t a = 0; a < ARM_DISTANCE; ++a) {
      mbarrier_arrive_expect_tx(&shm->bar_w[a], UP_W_TX_BYTES_TOTAL);
  ...
  for (uint32_t e = expert_start; e < expert_count; e += expert_stride) {
    ...
    // Per-slot parity state (SLOTS deep).  Every slot starts at parity 0:
    // slots [0, ARM_DISTANCE) are pre-armed (by the pre-loop for
    // expert_start, or by the previous expert's cross-expert stitch),
    // and the remaining slots are first armed inside this expert's
    // K-loop, so register 0 correctly expects physical-arrival 1 on each
    // slot's first try_wait.parity.
    uint32_t parity_w[SLOTS];
  #pragma unroll
    for (uint32_t i = 0; i < SLOTS; ++i) parity_w[i] = 0u;
  ...
  #pragma unroll
    ...
    // ── Main K-loop (SLOTS-deep weight pipeline) ───────────────────────────
    for (uint32_t s = 0; s < K_TILES; ++s) {
      // Consumer waits the slot armed A iters ago; launcher arms the slot
      // A iters ahead.  SLOTS is a power of two so the modulo is a mask.
      const uint32_t cur_slot = s & (SLOTS - 1u);
      const uint32_t arm_slot = (s + ARM_DISTANCE) & (SLOTS - 1u);
    ...
    #pragma unroll
      ...
      // ── Launcher: arm + TMA the slot A iters ahead ──────────────────────
      // Target logical iter is `s + ARM_DISTANCE`:
      //   * intra-expert if `s + A < K_TILES` → fetch THIS expert's
      //     iter (s+A);
      //   * else cross-expert → fetch the NEXT expert's iter
      //     `s + A - K_TILES` (in [0, A), since the loop's last A iters
      //     stitch the next expert's first A iters into slots [0, A)).
      // For SLOTS=2 (A=1) this is byte-identical to the old single-deep
      // stitch.  `K_TILES % SLOTS == 0` guarantees the stitched iters land
      // on the same slots the next expert's pre-armed pipeline expects.
      if (is_tma_launcher_thread<Dims>()) {
  #ifndef MONO_PROFILE_SKIP_PREFETCH_UP
        const uint32_t target_iter = s + ARM_DISTANCE;
        if (target_iter < K_TILES) {
