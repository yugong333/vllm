 ...
 * `my_expert` (warp-uniform).  This matches the production vLLM
 * `topk_softmax` kernel's tie-break ("lower expert index wins"), as opposed
 * to `warp_who_has`, which returns the lowest LANE — and since the
 * monokernel maps expert→lane as `expert % 32`, lowest-lane ≠ lowest-index
 * on a tie, causing the monokernel to route tied logits to a different
 * expert than every other backend.  A plain warp min-reduction over the
 * candidate experts (non-candidates contribute 0xFFFFFFFF) gives the
 * index-ordered winner directly, so no `warp_who_has` shuffle is needed.
 */
...
#pragma unroll
  for (int off = 16; off > 0; off >>= 1) {
    uint32_t other = __shfl_xor_sync(0xFFFFFFFFU, cand, off);
    cand = other < cand ? other : cand;
  }
  return cand;  // warp-uniform: the lowest expert id holding the max
}
...
__device__ void topK_BS8(uint32_t top_k, ScoringFunc scoring_func,
  ...
  #pragma unroll
      ...
      // Tie-break by LOWEST expert index (matches production topk_softmax),
      // not by lowest lane (warp_who_has) — see warp_min_expert_with_max.
      uint32_t winning_expert =
          warp_min_expert_with_max(max_val, warp_max, max_expert);
      // The winning weight is warp_max (all candidate lanes share it).
      float winning_weight = warp_max;
      if (tid == 0u) {
  ...
  #pragma unroll
    ...
    uint32_t winning_expert =
        warp_min_expert_with_max(max_val, warp_max, max_expert);
    // All candidate lanes share warp_max, so it IS the winner's score.
    float winning_score = warp_max;
...
__device__ void prepare_moe_topk_BS8(uint32_t batch_size, uint32_t top_k,
  ...
  // BLK = NUM_EXPERTS/32 contiguous u8 entries and zeroes its slice with the
  // widest aligned vector store that fits.  `expert_routed_count` follows
  // `alignas(16) bar_a[2]` so its base is >= 8-byte aligned; the per-lane base
  // `tid * BLK` then preserves that, so a uint{16,32,64} store (and a pair of
  // uint64 for BLK=16) is always naturally aligned.
  //   E=64→BLK=2 (u16), E=128→BLK=4 (u32), E=256→BLK=8 (u64),
  //   E=512→BLK=16 (2×u64).
  static_assert(BLK == 2u || BLK == 4u || BLK == 8u || BLK == 16u,
                "Vectorized zero supports BLK in {2,4,8,16} (NUM_EXPERTS in "
                "{64,128,256,512}); add a store width for other expert counts.");
  {
    ...
    } else {  // BLK == 16: two u64 (keeps 8-byte alignment, no uint4 16-align)
  ...
  // Vector-load this lane's BLK contiguous u8 counts.  The lane base
  // `tid * BLK` bytes is only BLK-byte aligned, so the load width MUST match
  // BLK (a fixed u64 load would over-read AND be misaligned for BLK<8 — e.g.
  // BLK=2 lane 1 is at byte 2).  BLK in {2,4,8,16} -> read {2,4,8,16} bytes.
  ...
  // `expert_slot_start` is u16; each lane owns BLK = NUM_EXPERTS/32 contiguous
  // entries (= BLK*2 bytes).  Pack two u16 per u32 and store with the widest
  // aligned vector that fits the lane slice.  Values are bounded by
  // `n_pairs <= 64`, so the u16 cast is safe.  Per-lane base `tid*BLK*2` B:
  //   BLK=2→4B(u32), 4→8B(u64=2×u32), 8→16B(uint4), 16→32B(2×uint4).
  // `expert_slot_start` is alignas(16), so all these are naturally aligned.
  // Sparse `experts[].id` writes stay scalar (non-contiguous across lanes).
  {
    static_assert(BLK == 2u || BLK == 4u || BLK == 8u || BLK == 16u,
                  "expert_slot_start packed store supports BLK in {2,4,8,16}.");
    uint32_t w[8];  // up to BLK/2 packed u32 words (BLK<=16 -> <=8)
  #pragma unroll
