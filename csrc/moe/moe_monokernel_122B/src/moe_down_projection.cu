...
__device__ inline void moe_load_down_wgmma_act_scale_per_expert(
  ...
  static_assert(ScaleHalves == Dims::N / CoreDims::DOWN_ACT_BLOCK_SIZE,
                "down-proj activation scale tile must have "
                "N/DOWN_ACT_BLOCK_SIZE blocks per token (one per up-block, "
                "full reduction dim)");

  // Per-up-block activation-quant size: 64 (35B) or 128 (122B).
  constexpr unsigned ACT_BLOCK = CoreDims::DOWN_ACT_BLOCK_SIZE;
...
__device__ inline void moe_down_projection_BS8_allexperts_wgmma_tma(
  ...
  static_assert(DOWN_COL_HALVES <= 3u,
                "DOWN_COL_TILE > 384 not supported by the WGMMA down-proj "
                "M-axis structure (final_d / w_down_scale / down_out are "
                "sized for up to 3 128-row halves).");
  ...
  #pragma unroll
    ...
    if (need_first_expert_prime && is_tma_launcher_thread<Dims>()) {
      ...
      // and issue one 128-row TMA per (kk-substep, h-half) atom.  The
      // descriptor's boxDim row dimension is pinned to 128 host-side
      // (`create_down_weight_tma_desc(... row_box=128)`) because the TMA
      // boxDim hardware cap is 256 and DOWN_COL_TILE=384 (122B) exceeds
      // it; each atom delivers 16 KB and the DOWN_COL_HALVES atoms of a
      // substep land contiguously in `w_down_wgmma`.  The consumer-side
      // WGMMA A descriptor (one 128-row sub-atom per WGMMA call) is
      // unchanged.
      //
      ...
      // One 128-row TMA per (kk-substep, h-half) atom.  The TMA boxDim
      // hardware cap is 256 rows, so DOWN_COL_TILE=384 (DOWN_COL_HALVES=3)
      // cannot be a single row_box; the descriptor is built with
      // row_box=128 and we issue DOWN_COL_HALVES atoms per substep at
      // GM col `base_col + h*128`, landing at SHM row
      // `(kk*DOWN_COL_HALVES + h)*128` (matches the consumer's
      // `atom_off_bytes` indexing).
    ...
    #pragma unroll
          ...
          // Activation scales are hoisted to a per-expert single-buffer
          // `a_down_scale[block][tok]`, one entry per up-block of
          // `DOWN_ACT_BLOCK_SIZE` reduction-dim (N) elements.  The WGMMA
          // K-chain splits each 128-K substep into a "lo" chunk
          // (K[0..63]) and "hi" chunk (K[64..127]); each maps to the
          // activation block covering its global-K start:
          //   global_k(s,kk)   = (s*K_SUBSTEPS_DOWN + kk) * 128
          //   block_lo = global_k          / DOWN_ACT_BLOCK_SIZE
          //   block_hi = (global_k + 64)   / DOWN_ACT_BLOCK_SIZE
          // 35B (block=64): block_hi == block_lo + 1 (legacy 64-K halves).
          // 122B (block=128): block_hi == block_lo (lo+hi share a scale).
          //
          // Layout note: the legacy `[tok][block]` layout caused a 2-way
          // bank conflict on these LDS sites; the `[block][tok]`
          // transpose (matching `MoE_SHM::act_scale`) puts every
          // (tok_02 ∈ {0,2,4,6}) lookup on a distinct bank within one
          // 32-B row → conflict-free.
