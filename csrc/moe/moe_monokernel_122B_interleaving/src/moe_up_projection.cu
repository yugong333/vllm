...
//   * `base_row_up = effective_bid * 128` (not *64); the block produces
//     128 gate + 128 up output features.
...
//     matches `W_WGMMA_M_TOTAL = 128 * K_SUBSTEPS_UP * UP_COL_HALVES`), and
//     its TMA `base_row` is `base_row_up + h*64` (the interleave packs
//     gate/up per 64-row block).
//   * Weight TMA lookahead is 2-deep (`UP_W_SLOTS == 2`): calc waits on
//     `bar_w[s & 1]`, launcher arms `bar_w[(s+1) & 1]`.
//   * The deferred SiLU+fp8 writeback reduces over 128 features (4 values
//     per lane: atom0 WG0/WG1 + atom1 WG0/WG1) for the block-128 quant.
...
//
// This fork targets the V2 (Pair_Layout) + DEFER epilogue only — the
// 122B Dims sets `USE_PAIR_LAYOUT = true`.
template <typename Dims>
__device__ inline void moe_up_projection_BS8_122B_wgmma_tma(
  ...
  #pragma unroll
    ...
    #pragma unroll
            ...
            // Scale-apply at the K=128 boundary.  Both atoms share the same
            ...
            const float ws_gate = ws_buf[kblk + 0u];
            const float ws_up = ws_buf[kblk + UP_SCALE_COLS];
            final_d[h][0] += chunk_d0 * ws_gate * as_02;
            final_d[h][1] += chunk_d1 * ws_gate * as_13;
            final_d[h][2] += chunk_d2 * ws_up * as_02;
            final_d[h][3] += chunk_d3 * ws_up * as_13;
          }
        }
  #endif
      }
      ...
      if (defer_iter && has_pending_writeback && is_prefetch_warp<Dims>()) {
        ...
        // 4 post_silu_scratch reads — atom0 (rows col, col+64), atom1
        // (rows 128+col, 128+64+col).
        const float pv[4] = {
  ...
  #pragma unroll
    ...
    // ── End-of-expert calc epilogue: combine + store BOTH atoms ────────────
    {
      if (is_calc) {
        ...
        // signal — compare gate/up against the Python reference GEMM1
        // for (tok0, expert experts[expert_start].id) BEFORE the SiLU /
        // rw / quant steps to localize a register↔feature mapping bug.
        if (blockIdx.x == 0 && threadIdx.x == 0 && e == expert_start) {
          printf(
              "[MONO_DBG] UP-WGMMA blk0 e0 id=%u tok0: "
              "atom0{gate=%.5f up=%.5f} atom1{gate=%.5f up=%.5f} "
              "base_row_up=%u\n",
  ...
  #pragma unroll
        for (uint32_t h = 0; h < HALVES; ++h) {
          ...
          float val1 = __fdividef(rw_odd * final_d[h][3] * final_d[h][1],
                                  1.0f + __expf(-final_d[h][1]));
          if (!store_even) val0 = 0.f;
          if (!store_odd) val1 = 0.f;
          const uint32_t row_in_tile =
              h * 128u + (is_wg1 ? 64u : 0u) + warp_in_wg * 8 + lane / 4;
          shm->partial_result.post_silu_scratch[row_in_tile][tok_even] = val0;
          shm->partial_result.post_silu_scratch[row_in_tile][tok_odd] = val1;
        }
